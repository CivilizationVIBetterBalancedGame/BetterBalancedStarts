...
function GeneratePlotTypes(world_age)
	...
	while done == false do
		...
		if (removedLand < iNumTotalTiles / 100) then
			...
			if (land_percent_plus >= target_percent and land_percent_minus <= target_percent + 10) then
				...
				-- or an island map
				if (iNumBiggestAreaTiles <= iNumTotalLandTiles * 0.6 and iNumBiggestAreaTiles >= iNumTotalLandTiles * 0.4) then
					done = true;
	...
	print("- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -");
	print("--- Detailed Continents landmass generation, Attempt#", iAttempts);
