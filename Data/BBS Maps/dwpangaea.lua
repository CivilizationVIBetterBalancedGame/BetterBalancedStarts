...
function GeneratePlotTypes(world_age)
	...
	while done == false do
		...
		if rift_dice < 1 then
			rift_dice = 1; -- 705: -1 looks like junk in testing
		end
		
		-- 705: Adding arg to disable polar land (2nd false below)
		-- Now using 2 passes for pangaea maps.  The first pass will generate the main landmass
		-- and the second pass will add tiny islands using high sea level.  The detail pass will
		-- bring the total number of land tiles back in line.		
		...
		-- 705: For Pangaea we can't just do a single pass since it generally looks bad. We'll use
		-- four passes in quadrants to add variety.
		...
		local args = {};
		args.iWaterPercent = detail_water - 5;
		args.iRegionWidth = math.ceil(g_iW/2);
		args.iRegionHeight = math.ceil(g_iH/2);
		args.iRegionWestX = math.floor(0);
		...
		args.iRegionHeight = math.ceil(g_iH/2);
		args.iRegionWestX = math.floor(0);
		args.iRegionSouthY = math.floor(g_iH/2);
		args.iRegionGrain = 4;
		args.iRegionPlotFlags = g_iFlags;
		args.iRegionFracXExp = 7;
		args.iRegionFracYExp = 6;
		...
		args.iRegionWidth = math.ceil(g_iW/2);
		args.iRegionHeight = math.ceil(g_iH/2);
		args.iRegionWestX = math.ceil(g_iW/2);
		args.iRegionSouthY = math.floor(0);
		args.iRegionGrain = 3;
		args.iRegionPlotFlags = g_iFlags;
		args.iRegionFracXExp = 7;
		...
		args.iRegionHeight = math.ceil(g_iH/2);
		args.iRegionWestX = math.ceil(g_iW/2);
		args.iRegionSouthY = math.ceil(g_iH/2);
		args.iRegionGrain = 4;
		args.iRegionPlotFlags = g_iFlags;
		args.iRegionFracXExp = 7;
		args.iRegionFracYExp = 6;
		...
		-- 705: On pangaea maps, if we had to remove too much polar land it probably looks
		-- really bad so lets just eliminate those maps now.
		
		if (removedLand < iNumTotalTiles / 110) then
			AreaBuilder.Recalculate();
			local biggest_area = Areas.FindBiggestArea(false);
			...
			if (land_percent_plus >= target_percent and land_percent_minus <= target_percent + 10) then
				...
				-- or an island map
				if (iNumBiggestAreaTiles >= iNumTotalLandTiles * 0.8) then
					done = true;
	...
	print("- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -");
	print("--- Detailed Pangaea landmass generation, Attempt#", iAttempts);
	...
	for x = 0, g_iW - 1 do
		for y = 0, g_iH - 1 do
			...
			if(plotTypes[i] == g_PLOT_TYPE_LAND or plotTypes[i] == g_PLOT_TYPE_HILLS) then
				...
				if(mountainCount > 4) then -- surrounded by mountains
					for direction = 0, DirectionTypes.NUM_DIRECTION_TYPES - 1, 1 do
