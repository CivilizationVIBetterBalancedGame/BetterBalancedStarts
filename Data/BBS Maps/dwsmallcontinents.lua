...
--	PURPOSE: Map script - Produces varied small continents.
------------------------------------------------------------------------------
...
function GeneratePlotTypes(world_age)
	...
	while done == false do
		...
		-- 705: On continent maps, if we had to remove too much polar land it probably looks
		...
		if (removedLand < iNumTotalTiles / 100) then
			...
			if (land_percent_plus >= target_percent and land_percent_minus <= target_percent + 10) then
				...
				-- or an island map
				if (iNumBiggestAreaTiles <= iNumTotalLandTiles * 0.33 and iNumBiggestAreaTiles >= iNumTotalLandTiles * 0.2) then
					done = true;
	...
	print("- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -");
	print("--- Detailed Small Continents landmass generation, Attempt#", iAttempts);
