...
function Hex:UpdateYields()
    ...
    self.ExtraYield = self.Science > 0 or self.Culture > 0 or self.Faith > 0;
...
function HexMap:TerraformAddRandomLux(hex)
    ...
    for _, idLux in pairs(possiblesLux) do
...
function HexMap:TerraformEmptyTile(hex)
    if self:TerraformEmptyTileRequirements(hex) then
...
function HexMap:TerraformTo4Yields(hex, forced)
    ...
    if hex:IsTundraLand() then
        return self:TerraformTo4YieldsTundra(hex)
    end
    -- Low yields upgrades
...
function HexMap:TerraformAdd1Food(hex, canMinusProd)
    ...
    if canMinusProd then
        -- Plain to grass
        if (hex.ResourceType == g_RESOURCE_NONE or hex.ResourceType == g_RESOURCE_DEER) and (hex.FeatureType == g_FEATURE_FOREST or hex.FeatureType == g_FEATURE_NONE) then
            if hex.TerrainType == g_TERRAIN_TYPE_PLAINS then
        ...
        -- Forest to jungle if possible
        if self:CanHaveJungle(hex) and IsPlainLand(hex.TerrainType) and hex.FeatureType == g_FEATURE_FOREST then
            return self:TerraformSetFeature(hex, g_FEATURE_JUNGLE, false);
        end
    end
    -- Can add jungle or bananas in jungle
    if self:CanHaveJungle(hex) and IsPlainLand(hex.TerrainType) then
        if hex.FeatureType == g_FEATURE_NONE then
            return self:TerraformSetFeature(hex, g_FEATURE_JUNGLE, false);
        elseif hex.FeatureType == g_FEATURE_JUNGLE then
            return self:TerraformSetResource(hex, g_RESOURCE_BANANAS);
        end
    end
    -- Add food resource on empty tiles
    if hex.ResourceType == g_RESOURCE_NONE and hex.FeatureType == g_FEATURE_NONE then
        if hex.TerrainType == g_TERRAIN_TYPE_GRASS then
            if rng <= 45 then
                return self:TerraformSetResource(hex, g_RESOURCE_RICE);
            elseif rng <= 90 then
...
function HexMap:TerraformAdd1Prod(hex, canMinusFood)
    if  g_RESOURCES_STRATEGICS[hex.ResourceType]
        or g_RESOURCES_LUX_LIST[hex.ResourceType]
        or hex:IsWater()
        or hex:IsFloodplains(true)
    ...
    if canMinusFood and hex.ResourceType == g_RESOURCE_NONE and (hex.FeatureType == g_FEATURE_FOREST or hex.FeatureType == g_FEATURE_NONE) then
        _Debug("TerraformAdd1Prod - Grass to plains")
        if hex.TerrainType == g_TERRAIN_TYPE_GRASS then
    ...
    -- Jungle to forest
    if canMinusFood and hex.FeatureType == g_FEATURE_JUNGLE and hex.ResourceType == g_RESOURCE_NONE then
        _Debug("TerraformAdd1Prod - Jungle to forest")
        return self:TerraformSetFeature(hex, g_FEATURE_FOREST);
    end
    if hex.ResourceType == g_RESOURCE_NONE then
        -- If already have forest, can add deer
        if hex.FeatureType == g_FEATURE_FOREST and (rng <= 15 or IsTundraLand(hex.TerrainType) ) then
            _Debug("TerraformAdd1Prod - Add deer")
            return self:TerraformSetResource(hex, g_RESOURCE_DEER);
        end
        if hex.FeatureType == g_FEATURE_JUNGLE and hex.TerrainType == g_TERRAIN_TYPE_PLAINS then
            _Debug("TerraformAdd1Prod - Plain jungle to plain hills jungle")
            return self:TerraformSetTerrain(hex, g_TERRAIN_TYPE_PLAINS_HILLS);
        end
        -- No feature
        if hex.FeatureType == g_FEATURE_NONE then
            -- Can add stone on flat or hills grassland
            if IsGrassLand(hex.TerrainType) and rng <= 15 then
                _Debug("TerraformAdd1Prod - Add stone")
                return self:TerraformSetResource(hex, g_RESOURCE_STONE);
            end
            -- Change flat to hills or forest by default
            _Debug("TerraformAdd1Prod - Add hills")
            if hex.TerrainType == g_TERRAIN_TYPE_GRASS then
                if rng <= 50 then
                    return self:TerraformSetFeature(hex, g_FEATURE_FOREST);
                else
            ...
            -- Default (hills) try add forest
            if self:TerraformSetFeature(hex, g_FEATURE_FOREST) then
                _Debug("TerraformAdd1Prod - Add forest")
                return true;
            end
        end
...
function HexMap:TerraformRemove1Prod(hex, canAddFood)
    ...
    if canAddFood and hex.ResourceType == g_RESOURCE_NONE and (hex.FeatureType == g_FEATURE_FOREST or hex.FeatureType == g_FEATURE_JUNGLE) then
        if hex.TerrainType == g_TERRAIN_TYPE_PLAINS then
            return self:TerraformSetTerrain(hex, g_TERRAIN_TYPE_GRASS);
        elseif hex.TerrainType == g_TERRAIN_TYPE_PLAINS_HILLS then
            return self:TerraformSetTerrain(hex, g_TERRAIN_TYPE_GRASS_HILLS);
...
function HexMap:FixedTerraformTo31(hex)
    if hex:IsTundraLand() or hex:IsSnowLand() or hex:IsDesertLand()
        or g_RESOURCES_STRATEGICS[hex.ResourceType]
        or g_RESOURCES_LUX_LIST[hex.ResourceType]
        or hex:IsWater()
        or hex:IsFloodplains(true)
    ...
    if self:CanHaveJungle(hex) then
        self:TerraformSetTerrain(hex, g_TERRAIN_TYPE_PLAINS_HILLS);
        self:TerraformSetFeature(hex, g_FEATURE_JUNGLE, false);
        self:TerraformSetResource(hex, g_RESOURCE_BANANAS, false);
        hex:UpdateYields()
        return true;
    end
   return false;
...
function HexMap:TerraformHex(hex, type, id, boolParam)
    if type == TerraformType[1] then
        return self:TerraformSetTerrain(hex, id);
    elseif type == TerraformType[2] then
        return self:TerraformSetFeature(hex, id, boolParam);
    elseif type == TerraformType[3] then
        return self:TerraformSetResource(hex, id, boolParam);
    ...
    elseif type == TerraformType[6] then
        return self:TerraformAdd1Food(hex, boolParam);
    elseif type == TerraformType[7] then
        return self:TerraformRemove1Food(hex, boolParam);
    elseif type == TerraformType[8] then
        _Debug("TerraformAdd1Prod from TerraformType8")
        return self:TerraformAdd1Prod(hex, boolParam);
    ...
    elseif type == TerraformType[11] then
        return self:TerraformAddRandomLux(hex);
...
function SpawnBalancing.new(hex, hexMap, civ)
    ...
    instance.MaxLuxInnerRingThreshold = 2;
    instance.MaxLuxOuterRingThreshold = 4;
    instance.MinLuxInnerRingThreshold = 1;
    instance.MinLuxOuterRingThreshold = 2;
...
function SpawnBalancing:TerraformRing6Deserts()
    if self.Civ.IsDesertBias then
        ...
        if self.Hex.IsOnRiver == false and self.Hex:IsNextToOasis() == false then
            _Debug("Desert bias need to add oasis");
            for _, h in pairs(self.RingTables[1].DESERT) do
                if self:TerraformHex(h, 1, TerraformType[1], g_TERRAIN_TYPE_DESERT, true) then
                    self:TerraformHex(h, 1, TerraformType[2], g_FEATURE_OASIS, true)
                    self:TerraformHex(h, 1, TerraformType[3], g_RESOURCE_NONE, true)
...
function SpawnBalancing:ApplyMinimalLandTiles(iMin, iMax)
    ...
    for _, h in pairs(self.RingTables[2].HIGH_YIELD_TILES) do
        if h.Food >= 4 then
            _Debug("ApplyMinimalLandTiles - Found a 4+ food tile ring 2 "..h:PrintXY())
            self:RelocateHex(2, 3, h);
        end
    end
    ...
    for i, _ in pairs(self.RingTables) do
        ...
        if i >= iMin then
            ...
            local minimumStandardTiles = self:GetMinimumStandardPerRing(i)
                ...
                if relocatedHex ~= nil and self:TerraformHex(relocatedHex, i, TerraformType[4]) then
                    ...
                    if #self.RingTables[i].MOUNTAINS > 0 then
                        ...
                        if mountainHex ~= nil then
                            ...
                            self:TerraformHex(mountainHex, i, TerraformType[10], 0, false, false)
                            _Debug("A mountain had to be terraformed ")
                            if self:TerraformRandomInRing(i, TerraformType[4], 0, false, false) then
...
function SpawnBalancing:GetMinimumStandardPerRing(i)
    if i == 1 and self.IsBCYActivated == false then
        return i + 1;
    end
    return i;
end
...
function SpawnBalancing:ApplyMinimalCoastalTiles()
    ...
    if validHarborTile == false and #self.RingTables[1].WATER_RF > 0 then
        ...
        self:RelocateRandomHexToNextRing(self.RingTables[1].WATER_RF, 1, 2)
    ...
    for _, h in pairs(self.RingTables[2].WATER) do
        if h.TerrainType == g_TERRAIN_TYPE_OCEAN then
            self:TerraformHex(h, 2, TerraformType[1], g_TERRAIN_TYPE_COAST, false)
        end
    end
    ...
    if foundFishR2 == false then
        if #self.RingTables[2].WATER_EMPTY > 0 then
            _Debug("Need to add a fish in empty hex in ring 2 for "..self.Civ.CivilizationLeader)
        ...
        elseif #self.RingTables[2].WATER_RF > 0 then
            ...
            local relocatedHex = self:RelocateRandomHexToNextRing(self.RingTables[2].WATER_RF, 2, 3)
            if relocatedHex then
                _Debug("Need to add a fish in "..relocatedHex:PrintXY().." in ring 2 for "..self.Civ.CivilizationLeader)
                self:TerraformHex(relocatedHex, 2, TerraformType[3], g_RESOURCE_FISH, false)
            end
        end
    end
    ...
    for _, h in pairs(self.RingTables[3].WATER) do
        if h.TerrainType == g_TERRAIN_TYPE_OCEAN then
            ...
            if rng <= 50 then
                self:TerraformHex(h, 3, TerraformType[1], g_TERRAIN_TYPE_COAST, false);
            end
        end
...
function SpawnBalancing:GetHexBaseFood(hex)
    local bonusFood = 0;
    local isTundra = hex:IsTundraLand()
    if isTundra then
        bonusFood = 1;
    end
...
function SpawnBalancing:CheckMinimumWorkable()
    ...
    for _, mainHex in pairs(self.RingTables[1].WORKABLE) do
        -- Tae standard and high yields
        if self:GetHexBaseFood(mainHex) + self:GetHexBaseProd(mainHex) >= 4 then
            for _, secondHex in pairs(self.RingTables[1].WORKABLE) do
    ...
    if #combinaisonList == 0 then
        ...
        -- Add a 2/2
        if self:TerraformRandomInRing(1, TerraformType[4], 0, false, false) then
    ...
    for _, comb in pairs(combinaisonList) do
        ...
        loop = 0;
        while comb.ProdDiff < 0 and loop <= 5 do
            ...
            -- Little rng to check the highest tile first
            if rng <= 20 then
                if self:TerraformHex(comb.Hex1, 1, TerraformType[8], 0, canRemoveFood) then
                    comb.FoodDiff = self:GetFoodDiff(comb.Hex1, comb.Hex2);
                    comb.ProdDiff = self:GetProdDiff(comb.Hex1, comb.Hex2);
                    _Debug("Added 1 prod in Hex1 "..tostring(comb.FoodDiff).." "..tostring(comb.ProdDiff))
                elseif self:TerraformHex(comb.Hex2, 1, TerraformType[8], 0, canRemoveFood) then
                    comb.FoodDiff = self:GetFoodDiff(comb.Hex1, comb.Hex2);
                    comb.ProdDiff = self:GetProdDiff(comb.Hex1, comb.Hex2);
                    _Debug("Added 1 prod in Hex2 "..tostring(comb.FoodDiff).." "..tostring(comb.ProdDiff))
                end
            else
                if self:TerraformHex(comb.Hex2, 1, TerraformType[8], 0, canRemoveFood) then
                    comb.FoodDiff = self:GetFoodDiff(comb.Hex1, comb.Hex2);
                    comb.ProdDiff = self:GetProdDiff(comb.Hex1, comb.Hex2);
                    _Debug("Added 1 prod in Hex2 "..tostring(comb.FoodDiff).." "..tostring(comb.ProdDiff))
                elseif self:TerraformHex(comb.Hex1, 1, TerraformType[8], 0, canRemoveFood) then
                    comb.FoodDiff = self:GetFoodDiff(comb.Hex1, comb.Hex2);
                    comb.ProdDiff = self:GetProdDiff(comb.Hex1, comb.Hex2);
                    _Debug("Added 1 prod in Hex1 "..tostring(comb.FoodDiff).." "..tostring(comb.ProdDiff))
                end
        ...
        if minimumFoodCheckOK and minimumProdCheckOK then
           ...
           --[[ _Debug("CheckMinimumWorkable - Try relocate ")
            if self:RelocateHex(1, 3, comb.Hex2) and self:TerraformHex(comb.Hex2, 1, TerraformType[4], 0, false, false) then
                self:PlaceRelocatedHexOnRing(3);
                comb.FoodDiff = self:GetFoodDiff(comb.Hex1, comb.Hex2);
                comb.ProdDiff = self:GetProdDiff(comb.Hex1, comb.Hex2);
            ...
            -- Try to add a last forced terraform on a bonus tile
            --if self:TerraformRandomInRing(1, TerraformType[4], 0, false, true) then
            --    _Debug("last forced terraform on a tile to 22")
             --   return
            --end
        end
    end
...
function SpawnBalancing:CheckLuxThreshold()
    local innerRingLuxCount = 0;
    local outerRingLuxCount = 0;
    local innerLuxTable = {};
    local outerLuxTable = {};
    local i = 1;
    while i <= 6 do
        for _,h in pairs(self.RingTables[i].HexRings) do
            if h:HasLux() then
                if i <= 2 then
                    table.insert(innerLuxTable, h);
                    innerRingLuxCount = innerRingLuxCount + 1;
                    -- Delete extra lux in ring 1
                    if (innerRingLuxCount == 2 and i == 1) then
                        self:RelocateHex(1, 3, h);
                        innerRingLuxCount = innerRingLuxCount - 1;
                        self:PlaceRelocatedHexOnRing(3);
                    end
                    if (innerRingLuxCount > self.MaxLuxInnerRingThreshold) then
                        self:TerraformHex(h, i, TerraformType[3], g_RESOURCE_NONE, false);
                        _Debug("CheckLuxThreshold deleted a lux on ", h:PrintXY(), " Ring = ", i)
                    end
                else
                    table.insert(outerLuxTable, h);
                    outerRingLuxCount = outerRingLuxCount + 1;
                    if outerRingLuxCount > self.MaxLuxOuterRingThreshold then
                        self:TerraformHex(h, i, TerraformType[3], g_RESOURCE_NONE, false);
                        _Debug("CheckLuxThreshold deleted a lux on ", h:PrintXY(), " Ring = ", i)
                    end
                end
    ...
    _Debug("CheckLuxThreshold innerRing = ", innerRingLuxCount, " outerRing = ", outerRingLuxCount, " Threshold = ", innerRingLuxCount < self.MaxLuxInnerRingThreshold)
    if innerRingLuxCount< self.MinLuxInnerRingThreshold then
        self:TerraformInRingsRandomOrder(1, 2, TerraformType[11], 0, false, false)
    end
    if outerRingLuxCount< self.MinLuxOuterRingThreshold then
        self:TerraformInRingsRandomOrder(4, 6, TerraformType[11], 0, false, true)
    end
            ...
            if isRelocated == false and #self.RingTables[i].LOW_YIELD_TILES > 0 then
                ...
                for _, hex in pairs(randomLowYields) do
                    ...
                    elseif hexData.ResourceId ~= nil and g_RESOURCES_STRATEGICS[hexData.ResourceId] then
                        self:TerraformHex(hex, i, TerraformType[1], hexData.TerrainId, true);
                        self:TerraformHex(hex, i, TerraformType[2], hexData.FeatureId, true);
                        if self:TerraformHex(hex, i, TerraformType[3], hexData.ResourceId, true) then
                            _Debug("Relocated feat = "..tostring(hexData.FeatureId).." res = "..tostring(hexData.ResourceId).." on "..hex:PrintXY())
                            totalRelocating = totalRelocating - 1;
                            isRelocated = true;
                        end
...
function SpawnBalancing:TerraformInRingsRandomOrder(iMin, iMax, terraformType, id, needToBeWalkableTo, forcedLastRing)
    ...
    for _, iRing in pairs(order) do
        ...
        if self:TerraformRandomInRing(iRing, terraformType, id, needToBeWalkableTo, forced) then
            return;
        end
    end
end
...
function SpawnBalancing:TerraformRandomInRing(i, terraformType, id, needToBeWalkableTo, forced)
    ...
    if #self.RingTables[i].LOW_YIELD_TILES > 0 then
        ...
        for _, hex in pairs(listLowYieldsTiles) do
            if needToBeWalkableTo == false or (needToBeWalkableTo and self.Hex:IsWalkableInRange(hex, i)) then
                if self:TerraformHex(hex, i, terraformType, id, false, true) then
        ...
        if #listLowYieldsTiles > 0 and forced then
            ...
            for _, hex in pairs(listLowYieldsTiles) do
                if needToBeWalkableTo == false or (needToBeWalkableTo and self.Hex:IsWalkableInRange(hex, i)) and hex:IsFloodplains(true) == false then
...
function SpawnBalancing:CanRelocateStrategics(resId, i)
    ...
    if (resId == RESOURCE_HORSES and i < maxRingAllowedForHorses) or (resId == RESOURCE_IRON and i < maxRingAllowedForIron) then
        return true;
    end
    return false;
end
...
function SpawnBalancing:ApplyGaranteedStrategics()
    ...
    if horsesOK == false then
        ...
        self:TerraformInRingsRandomOrder(1, 2, TerraformType[3], g_RESOURCE_HORSES, true, true);
    end
    if ironOK == false then
        print("Adding iron 2-3")
        self:TerraformInRingsRandomOrder(2, 3, TerraformType[3], g_RESOURCE_IRON, true, true);
    end
...
function SpawnBalancing:TerraformHex(hex, ring, terraformType, id, forced, blockTerraform)
    local blockedTerraform = forced == false and blockTerraform and Contains(self.TerraformedTiles, hex);
    if blockedTerraform == false and self.HexMap:TerraformHex(hex, terraformType, id, forced) then
...
function BalanceAllCivYields(spawns)
    ...
    for _, spawn in pairs(spawns) do
        local includingExtraYields = 0;
        local innerRingWorkable, innerRingHighYields, innerRingFood, innerRingProd = spawn:GetInnerRingWorkable();
        ...
        local outerRingWorkable, outerRingHighYields, outerRingFood, outerRingProd = spawn:GetOuterRingWorkable();
    ...
    meanInnerRingFood = math.floor((meanInnerRingFood / #spawns) + 0.5);
    meanInnerRingProd = math.floor((meanInnerRingProd / #spawns) + 0.5);
    meanOuterRingFood = math.floor((meanOuterRingFood / #spawns) + 0.5);
    meanOuterRingProd = math.floor((meanOuterRingProd / #spawns) + 0.5);
    meanInnerRingHighYields = math.floor((meanInnerRingHighYields / #spawns) + 0.5);
    meanOuterRingHighYields = math.floor((meanOuterRingHighYields / #spawns) + 0.5);
    meanInnerRingWorkable = math.floor((meanInnerRingWorkable / #spawns) + 0.5);
    meanOuterRingWorkable = math.floor((meanOuterRingWorkable / #spawns) + 0.5);
    _Debug("meanInnerRingFood = "..tostring(meanInnerRingFood));
    _Debug("meanInnerRingProd = "..tostring(meanInnerRingProd));
    _Debug("meanInnerRingWorkable = "..tostring(meanInnerRingWorkable));
    _Debug("meanInnerRingHighYields = "..tostring(meanInnerRingHighYields));
    _Debug("meanOuterRingFood = "..tostring(meanOuterRingFood));
    _Debug("meanOuterRingProd = "..tostring(meanOuterRingProd));
    _Debug("meanOuterRingWorkable = "..tostring(meanOuterRingWorkable));
    _Debug("meanOuterRingHighYields = "..tostring(meanOuterRingHighYields));
...
function SpawnBalancing:GetInnerRingWorkable()
    local innerRingFood = 0;
    local innerRingProd = 0;
    local highYields1 = #self.RingTables[1].HIGH_YIELD_TILES + #self.RingTables[1].HIGH_EXTRA_YIELDS;
    local highYields2 = #self.RingTables[2].HIGH_YIELD_TILES + #self.RingTables[2].HIGH_EXTRA_YIELDS;
    local totalInnerTiles = 18 - #self.RingTables[1].WATER_EMPTY + #self.RingTables[1].MOUNTAINS;

    local ring1workable = #self.RingTables[1].STANDARD_YIELD_TILES  + #self.RingTables[1].WATER_FISH_OR_LUX
            + #self.RingTables[1].HIGH_EXTRA_YIELDS + #self.RingTables[1].HIGH_YIELD_TILES;
    for _, h in pairs(self.RingTables[1].WORKABLE) do
        innerRingFood = innerRingFood + self:GetHexBaseFood(h);
        innerRingProd = innerRingProd + self:GetHexBaseProd(h);
    end
    local ring2workable = #self.RingTables[2].STANDARD_YIELD_TILES + #self.RingTables[2].WATER_FISH_OR_LUX
            + #self.RingTables[2].HIGH_EXTRA_YIELDS + #self.RingTables[2].HIGH_YIELD_TILES;
    for _, h in pairs(self.RingTables[2].WORKABLE) do
        innerRingFood = innerRingFood + self:GetHexBaseFood(h);
        innerRingProd = innerRingProd + self:GetHexBaseProd(h);
    end
    print(tostring(self.Civ.CivilizationLeader).." GetInnerRingWorkable R1 : "..tostring(ring1workable).." R2 : "..tostring(ring2workable))
    return ring1workable + ring2workable, highYields1 + highYields2, innerRingFood, innerRingProd;
end

function SpawnBalancing:GetOuterRingWorkable()
    local outerRingFood = 0;
    local outerRingProd = 0;
    local highYields3 = #self.RingTables[3].HIGH_YIELD_TILES + #self.RingTables[3].HIGH_EXTRA_YIELDS;
    local highYields4 = #self.RingTables[4].HIGH_YIELD_TILES + #self.RingTables[4].HIGH_EXTRA_YIELDS;
    local ring3workable = #self.RingTables[3].STANDARD_YIELD_TILES + #self.RingTables[3].WATER_FISH_OR_LUX
        + #self.RingTables[3].HIGH_EXTRA_YIELDS + #self.RingTables[3].HIGH_YIELD_TILES;
    for _, h in pairs(self.RingTables[3].WORKABLE) do
        outerRingFood = outerRingFood + self:GetHexBaseFood(h);
        outerRingProd = outerRingProd + self:GetHexBaseProd(h);
    end
    local ring4workable = #self.RingTables[4].STANDARD_YIELD_TILES + #self.RingTables[4].WATER_FISH_OR_LUX
            + #self.RingTables[4].HIGH_EXTRA_YIELDS + #self.RingTables[4].HIGH_YIELD_TILES;
    for _, h in pairs(self.RingTables[4].WORKABLE) do
        outerRingFood = outerRingFood + self:GetHexBaseFood(h);
        outerRingProd = outerRingProd + self:GetHexBaseProd(h);
    end
    print(tostring(self.Civ.CivilizationLeader).." GetInnerRingWorkable R3 : "..tostring(ring3workable).." R4 : "..tostring(ring4workable))
    return ring3workable + ring4workable, highYields3 + highYields4, outerRingFood, outerRingProd;
end
    ...
    while math.abs(self.AimedCoastalScore - self.CoastalScore) > CoastalScoring.COASTAL_MARGIN do
        ...
        for _, h in pairs(waterRFRing2) do
            if h.ResourceType == g_RESOURCE_FISH and h.FeatureType == g_FEATURE_NONE
                ...
                self:TerraformHex(h, 2, TerraformType[2], g_FEATURE_REEF, true);
                self:TerraformHex(h, 2, TerraformType[3], g_RESOURCE_FISH, true);
        ...
        if stepDone == false then
            ...
            for _, h in pairs(waterRFRing3) do
                if h.ResourceType == g_RESOURCE_FISH and h.FeatureType == g_FEATURE_NONE
                    ...
                    self:TerraformHex(h, 3, TerraformType[2], g_FEATURE_REEF, true);
                    self:TerraformHex(h, 3, TerraformType[3], g_RESOURCE_FISH, true);
        ...
        if stepDone == false then
            ...
            for _, h in pairs(waterEmptyRing2) do
                ...
                elseif self:CanAddToCoastalScore(CoastalScoring.CRABS_R2) then
                    self:TerraformHex(h, 2, TerraformType[3], g_RESOURCE_CRABS, true);
        ...
        if stepDone == false then
            ...
            for _, h in pairs(waterEmptyRing3) do
                ...
                elseif self:CanAddToCoastalScore(CoastalScoring.CRABS_R3) then
                    self:TerraformHex(h, 3, TerraformType[3], g_RESOURCE_CRABS, true);
    ...
    while math.abs(self.AimedCoastalScore - self.CoastalScore) > CoastalScoring.COASTAL_MARGIN do
        ...
        for _, h in pairs(waterRFRing2) do
            if h.ResourceType == g_RESOURCE_FISH and h.FeatureType == g_FEATURE_REEF then
                if self:CanAddToCoastalScore(- CoastalScoring.FISH_REEF_R2) and self:CanNerfFishR2() then
                    self:TerraformHex(h, 2, TerraformType[2], g_FEATURE_NONE, true);
                    self:TerraformHex(h, 2, TerraformType[3], g_RESOURCE_NONE, true);
                    self:UpdateCoastalScore(- CoastalScoring.FISH_REEF_R2)
                    stepDone = true;
        ...
        if stepDone == false then
            ...
            for _, h in pairs(waterRFRing3) do
                if h.ResourceType == g_RESOURCE_FISH and h.FeatureType == g_FEATURE_REEF then
                    if self:CanAddToCoastalScore(- CoastalScoring.FISH_REEF_R3) then
                        self:TerraformHex(h, 3, TerraformType[2], g_FEATURE_NONE, true);
                        self:TerraformHex(h, 3, TerraformType[3], g_RESOURCE_NONE, true);
                        self:UpdateCoastalScore(- CoastalScoring.FISH_REEF_R3)
                        stepDone = true;
