...
function isHillAble(x, y)
    ...
    if (resource == 10 or resource == 12 or resource == 18 or resource == 20 or resource == 22 or resource == 24 or
        resource == 27 or resource == 28 or resource == 30 or resource == 31 or resource == 42 or resource == 44 or
        resource == 45 or resource == 53) then
        return -1;
    end
...
function toTwoTwo(x, y, canStone, canSheep, canBanana)
    ...
    if (resource ~= 0 and resource ~= 4 and resource ~= 7 and resource ~= 8 and resource ~= 11 and resource ~= 14 and
        resource ~= 15 and resource ~= 16 and resource ~= 4 and resource ~= 19 and resource ~= 25 and resource ~= 27 and
        resource ~= 33) then
        cleanResource = true;
    end
...
function BBS_Script()
    ...
    if currentTurn == GameConfiguration.GetStartTurn() then
        ...
        print("Init: Global Parameters: Major Civs Buffer:",
            GlobalParameters.START_DISTANCE_MAJOR_CIVILIZATION - GlobalParameters.START_DISTANCE_RANGE_MAJOR)
        ...
        if (force_remap == true and bError_proximity == true) then
            ...
            Game:SetProperty("BBS_DISTANCE_ERROR",
                "Minimum distances between civilizations could not be met. You must remap as per CPL rules.");
            print("Init: Exit");
            return
        end
        ...
        for i = 1, majorCount do
            ...
            for j = 3, fixedRing do
                ...
                for _, element in ipairs(list) do
                    ...
                    if (mapTerrainCode[xIndex][yIndex] == 0 or mapTerrainCode[xIndex][yIndex] == 1 or
                        mapTerrainCode[xIndex][yIndex] == 3 or mapTerrainCode[xIndex][yIndex] == 4) then -- we are looking at plain/grassland only
        ...
        for i = 1, major_count do
            ...
            if (startPlot ~= nil) then
                ...
                __Debug("Major Start X: ", majList[i].plotX, "Major Start Y: ", majList[i].plotY, "Player: ",
                    major_table[i], " ", majList[i].leader, majList[i].civ);
            end
        ...
        if debug_balancing == false then
            ...
            for i = 1, major_count do
                if major_table[i] ~= nil then
                    if Players[major_table[i]] ~= nil and Players[major_table[i]]:GetTeam() ~= nil and majList[i] ~= nil then
                        print("Player ID:", major_table[i], " Team:", Players[major_table[i]]:GetTeam(), majList[i].civ,
                            majList[i].leader);
                    else
                ...
                for i = 1, major_count do
                    ...
                    if (majList[i] ~= nil) then
                        if (majList[i].leader ~= "LEADER_SPECTATOR") then
                            ...
                            if ((majList[i].snow_start + majList[i].snow_inner + majList[i].snow_outer) > 6 and
                                IsTundraCiv(majList[i].civ) == false) or
                                ((majList[i].snow_start + majList[i].snow_inner + majList[i].snow_outer) > 2 and
                                    (majList[i].water_start + majList[i].water_inner + majList[i].water_outer) > 4 and
                                    IsTundraCiv(majList[i].civ) == false) then
                                __Debug("Terraforming Polar Start X: ", majList[i].plotX, "Start Y: ", majList[i].plotY,
                                    "Player: ", i, " ", majList[i].leader, majList[i].civ);
                                Terraforming(Map.GetPlot(majList[i].plotX, majList[i].plotY), iBalancingThree, 0);
                            end

                            if ((majList[i].desert_outer + majList[i].desert_inner + majList[i].desert_start) > 6 and
                                IsDesertCiv(majList[i].civ) == false) or
                                ((majList[i].desert_outer + majList[i].desert_inner + majList[i].desert_start) > 2 and
                                    (majList[i].water_start + majList[i].water_inner + majList[i].water_outer) > 4 and
                                    IsDesertCiv(majList[i].civ) == false) then
                                if (IsTundraCiv(majList[i].civ) == true) then
                                    __Debug("Terraforming Desert Start X: ", majList[i].plotX, "Start Y: ",
                                        majList[i].plotY, "Player: ", i, " ", majList[i].leader, majList[i].civ);
                            ...
                            if (IsDesertCiv(majList[i].civ) == false) and (IsTundraCiv(majList[i].civ) == false) and
                                (majList[i].desert_outer + majList[i].desert_inner + majList[i].desert_start +
                                    majList[i].snow_start + majList[i].snow_inner + majList[i].snow_outer) > 4 then
                                __Debug("Terraforming Mixed Start X: ", majList[i].plotX, "Start Y: ", majList[i].plotY,
                                    "Player: ", i, " ", majList[i].leader, majList[i].civ);
                                Terraforming(Map.GetPlot(majList[i].plotX, majList[i].plotY), iBalancingThree, 0);
                            end
                            ...
                            if (IsDesertCiv(majList[i].civ) == true) then -- Now forces to Terraform Mali to counterbalance the lower amount of deserts on the map
                                __Debug("Mali Terraforming Start X: ", majList[i].plotX, "Start Y: ", majList[i].plotY,
                                    "Player: ", i, " ", majList[i].leader, majList[i].civ);
                                Terraforming(Map.GetPlot(majList[i].plotX, majList[i].plotY), iBalancingThree, 2);
                            end

                            if (IsTundraCiv(majList[i].civ) == true) then
                                __Debug("Terraforming Start X: ", majList[i].plotX, "Start Y: ", majList[i].plotY,
                                    "Player: ", i, " ", majList[i].leader, majList[i].civ);
                                Terraforming(Map.GetPlot(majList[i].plotX, majList[i].plotY), iBalancingThree, 1);
                            end
            ...
            for i = 1, major_count do
                ...
                if (majList[i] ~= nil) then
                    if (majList[i].leader ~= "LEADER_SPECTATOR" and majList[i].leader ~= "LEADER_PACHACUTI") then
                        if (((majList[i].impassable_start + majList[i].impassable_inner + majList[i].impassable_outer) >=
                            12) or
                            ((majList[i].impassable_start + majList[i].impassable_inner + majList[i].impassable_outer) >=
                                8 and (majList[i].water_start + majList[i].water_inner + majList[i].water_outer) >= 4)) then
                            -- Check for Mountain Start
                            __Debug("Mountain Terraforming Start X: ", majList[i].plotX, "Start Y: ", majList[i].plotY,
                                "Player: ", i, " ", majList[i].leader, majList[i].civ);
                            Terraforming_Mountain(Map.GetPlot(majList[i].plotX, majList[i].plotY), 0)

                        end
                    end
                    if (majList[i].leader == "LEADER_PACHACUTI" and
                        (majList[i].impassable_start + majList[i].impassable_inner + majList[i].impassable_outer) < 2) then
                        __Debug("Mountain Terraforming Start X: ", majList[i].plotX, "Start Y: ", majList[i].plotY,
                            "Player: ", i, " ", majList[i].leader, majList[i].civ);
            ...
            for i = 1, major_count do
                ...
                if (majList[i] ~= nil) then
                    if (majList[i].leader ~= "LEADER_SPECTATOR") then
                        if (((majList[i].impassable_start + majList[i].water_start) > 4) and majList[i].leader ~=
                            "LEADER_PACHACUTI") then
                            -- Check for Walled-in
                            __Debug("Walled-In Start X: ", majList[i].plotX, "Start Y: ", majList[i].plotY, "Player: ",
                                i, " ", majList[i].leader, majList[i].civ);
            ...
            if string.lower(mapName) ~= "tilted_axis.lua" then
                for iPlotIndex = 0, Map.GetPlotCount() - 1, 1 do
                    ...
                    if (pPlot:GetY() > gridHeight / 6 and pPlot:GetY() < gridHeight * 4 / 9) or
                        (pPlot:GetY() > 5 * gridHeight / 9 and pPlot:GetY() < gridHeight * 5 / 6) then
                        if rng < 0.55 then
                            if pPlot:IsImpassable() == false and pPlot:IsWater() == false and pPlot:GetResourceType() ==
                                -1 and pPlot:GetFeatureType() == -1 and pPlot:GetTerrainType() ~= 7 and
                                pPlot:GetTerrainType() ~= 6 and pPlot:GetTerrainType() ~= 7 and pPlot:GetTerrainType() ~=
                                12 and pPlot:GetTerrainType() ~= 13 then
                ...
                if (majList[i] ~= nil) then
                    if (majList[i].leader ~= "LEADER_SPECTATOR") then
                        ...
                        if (wplot:IsFreshWater() == false) then
                            -- Fix No Water
                            __Debug("Water Terraforming Start X: ", majList[i].plotX, "Start Y: ", majList[i].plotY,
                                "Player: ", i, " ", majList[i].leader, majList[i].civ); -- put a print to catch the error in non debug mode
            ...
            if (bStrategic_only == false) then
                ...
                for i = 1, major_count do
                    if (majList[i] ~= nil) then
                        if (majList[i].leader ~= "LEADER_SPECTATOR") then
                            if (majList[i].food_spawn_start < ((avgFood) * (1 - dispersion * 3)) or
                                majList[i].food_spawn_start < minFood) then
                                __Debug("Need to adjust: ", majList[i].leader, majList[i].food_spawn_start, "Min Food:",
                                    minFood)
                                ...
                                elseif (majList[i].impassable_start > 1) then
                                    ...
                                    if (((majList[i].civ == "CIVILIZATION_INCA") and
                                        (math.floor(avgFood - majList[i].food_spawn_start) < 4)) or
                                        ((majList[i].civ == "CIVILIZATION_MAPUCHE") and
                                            (math.floor(avgFood + iBalancingTwo - majList[i].food_spawn_start) < 4))) then
                                        __Debug("Food balancing:", majList[i].leader,
                                            "Mountain Civ Detected: No need to re-balance");
                                    else
                                        __Debug("Food balancing:", majList[i].leader, "Start Mountains re-Balancing");
                                        __Debug("Food balancing: Food missing:", math.max(
                                            math.floor(avgFood - majList[i].food_spawn_start),
                                            minFood - majList[i].food_spawn_start));
                                        count = 0;
                                        for j = 1, math.max(math.floor(avgFood + 1 - majList[i].food_spawn_start),
                                            minFood - majList[i].food_spawn_start, 5) do
                                            if ((AddBonusFood(Map.GetPlot(majList[i].plotX, majList[i].plotY),
                                                iBalancingThree, 3, majList[i].harborPlot) == false)) then
                                                count = count + 1
                                                if (count == 3) then
                                                    __Debug("Food balancing:", majList[i].leader, "Will grand a luxury");
                                                    AddLuxuryStarting(Map.GetPlot(majList[i].plotX, majList[i].plotY),
                                                        "food")
                                                    count = count + 1;
                                                end
                                            end
                                    ...
                                    for j = 1, math.max(math.floor(avgFood + 1 - majList[i].food_spawn_start),
                                        ...
                                        if (majList[i].civ == "CIVILIZATION_MALI" and majList[i].desert_start > 0) then
                                            ...
                                            if count < 4 then
                                                if ((AddBonusFood(Map.GetPlot(majList[i].plotX, majList[i].plotY),
                                                    iBalancingThree, 2, majList[i].harborPlot) == false)) then
                                                    count = count + 1;
                                                end
                                            elseif count == 3 then
                                                __Debug("Food balancing:", majList[i].leader,
                                                    "Will grand a luxury (Desert Start)");
                                                AddLuxuryStarting(Map.GetPlot(majList[i].plotX, majList[i].plotY),
                                                    "food")
                                            else
                                                __Debug("Food balancing:", majList[i].leader,
                                                    "No longer grant anything to avoid overloading the spawn (Desert Start)");

                                            end
                                        elseif (majList[i].civ == "CIVILIZATION_RUSSIA" or majList[i].civ ==
                                            "CIVILIZATION_CANADA" and majList[i].snow_start > 2) then
                                            __Debug("Food balancing:", majList[i].leader, "Tundra start detected");
                                            if ((AddBonusFood(Map.GetPlot(majList[i].plotX, majList[i].plotY),
                                                iBalancingThree, 1, majList[i].harborPlot) == false)) then
                                                count = count + 1;
                                                if (count == 3) then
                                                    __Debug("Food balancing:", majList[i].leader,
                                                        "Will grand a luxury (Tundra Start)");
                                                    AddLuxuryStarting(Map.GetPlot(majList[i].plotX, majList[i].plotY),
                                                        "food")
                                                    count = count + 1;
                                                end
                                        ...
                                        elseif (majList[i].plains > 5) then
                                            ...
                                            if count_2 < 3 then
                                                if ((AddBonusFood(Map.GetPlot(majList[i].plotX, majList[i].plotY),
                                                    iBalancingThree, 0, majList[i].harborPlot) == false)) then
                                                    count = count + 1;
                                                    if (count == 3) then
                                                        __Debug("Food balancing:", majList[i].leader,
                                                            "Will grand a luxury (Plain Start)");
                                                        AddLuxuryStarting(
                                                            Map.GetPlot(majList[i].plotX, majList[i].plotY), "food")
                                                        count = count + 1;
                                                    end
                                                    if (count == 5) then
                                                        __Debug("Food balancing:", majList[i].leader,
                                            ...
                                            elseif count_2 == 3 then
                                                ...
                                                __Debug("Food balancing:", majList[i].leader,
                                                    "No longer grant anything to avoid overloading the spawn (Plain Start)");
                                            end
                                        else
                    ...
                    if (majList[i] ~= nil) then
                        if (majList[i].leader ~= "LEADER_SPECTATOR") then
                            if (Map.GetPlot(majList[i].plotX, majList[i].plotY):IsCoastalLand() == true) then
                                ...
                                __Debug("--------------");
                                __Debug("Coastal Terraforming Start X: ", majList[i].plotX, "Start Y: ",
                                    majList[i].plotY, "Player: ", i, " ", majList[i].leader, majList[i].civ);
                                ...
                                if (majList[i].isFullCoastal == true) then
                                    ...
                                    __Debug("Coastal: finished counting the civ minimal score:",
                                        majList[i].minCoastalScore);
                                end
                ...
                for i = 1, major_count do
                    if (majList[i] ~= nil) then
                        if (majList[i].leader ~= "LEADER_SPECTATOR") then
                            if ((majList[i].hill_start * 2 + majList[i].hill_inner) <
                                ((avgHill + 1) * (1 - dispersion * 2))) then
                                ...
                                if (Map.GetPlot(majList[i].plotX, majList[i].plotY):IsWater() == true) then
                                    ...
                                    __Debug("Prod balancing: Hills missing:", math.floor(
                                        (avgHill + iBalancingTwo - (majList[i].hill_start + majList[i].hill_inner)) / 2));

                                    if (math.floor((avgHill + iBalancingTwo -
                                                       (majList[i].hill_start + majList[i].hill_inner)) / 2) > 0) then
                                        count = 0;
                                        for j = 1, math.floor((avgHill + iBalancingTwo -
                                                                  (majList[i].hill_start + majList[i].hill_inner)) / 2) do

                                            if (AddHills(Map.GetPlot(majList[i].plotX, majList[i].plotY),
                                                iBalancingThree, 0) == false) then
                                                count = count + 1;

                                                if (count == math.min(3 - iBalancingTwo, 1)) then
                                                    __Debug("Hill balancing:", majList[i].leader,
                                                        "Will Grant something to be defined");
                                                end
                                            end
                ...
                for i = 1, major_count do
                    ...
                    if (majList[i] == nil or majList[i].leader == "LEADER_SPECTATOR") then
                        ...
                        if (majList[i].civ == "CIVILIZATION_INCA") then
                            majList[i].prod_spawn_start = majList[i].prod_spawn_start + tempEval[9] + tempEval[13] *
                                                              0.25 -- Adjust for Mountains more
                        end
                ...
                for i = 1, major_count do
                    if (majList[i] ~= nil) then
                        if (majList[i].leader ~= "LEADER_SPECTATOR") then
                            if (majList[i].prod_spawn_start < ((avgProd + iBalancingTwo) * (1 - dispersion)) or
                                majList[i].prod_spawn_start < minProd) then
                                __Debug("Prod balancing: Need to adjust: ", majList[i].leader,
                                    majList[i].prod_spawn_start, "Min Prod:", minProd)
                                ...
                                if (Map.GetPlot(majList[i].plotX, majList[i].plotY):IsWater() == true) then
                                    ...
                                    __Debug("Prod balancing: Prod missing:", math.max(
                                        math.floor((avgProd + iBalancingTwo - majList[i].prod_spawn_start)),
                                        minProd - majList[i].prod_spawn_start));

                                    if (math.max(math.floor((avgProd + iBalancingTwo - majList[i].prod_spawn_start)),
                                        minProd - majList[i].prod_spawn_start) > 0) then
                                        count = 0;
                                        for j = 1, math.max(math.floor(
                                            (avgProd + iBalancingTwo + 1 - majList[i].prod_spawn_start)),
                                            minProd - majList[i].prod_spawn_start) do

                                            if (majList[i].civ == "CIVILIZATION_INCA") then

                                                if (AddBonusProd(Map.GetPlot(majList[i].plotX, majList[i].plotY),
                                                    iBalancingThree, 3) == false) then
                                                    count = count + 1;

                                                    if (count == math.min(3 - iBalancingTwo, 1)) then
                                                        __Debug("Prod balancing:", majList[i].leader,
                                                            "Will try to grant a luxury");
                                                        AddLuxuryStarting(
                                                            Map.GetPlot(majList[i].plotX, majList[i].plotY), "prod");
                                                        count = count + 1;
                                                    end
                                ...
                                __Debug("Prod balancing: No Need to adjust: ", majList[i].leader,
                                    majList[i].prod_spawn_start)
                            end
                        end
                    end
                end
                            ...
                            if (majList[i].leader ~= "LEADER_SPECTATOR") then
                                if (majList[i].prod_spawn_start > ((avgProd + iBalancingTwo) * (1 + dispersion))) then
                                    ...
                                    if (math.floor(majList[i].prod_spawn_start - (avgProd + iBalancingTwo) - 1) > 0) then
                                        for j = 1, math.floor(
                                            majList[i].prod_spawn_start - (avgProd + iBalancingTwo) - 1) do
                                            RemoveProd(Map.GetPlot(majList[i].plotX, majList[i].plotY));
                                        end
                                    end
                                else
                                    __Debug("No need to adjust Production Down: ", majList[i].leader,
                                        majList[i].prod_spawn_start);
                                end
                                local max_score = 5.5
                                Terraforming_Cap_Yield(Map.GetPlot(majList[i].plotX, majList[i].plotY), max_score, 18,
                                    35);
                            end
                        end
                    end
                ...
                if (iStartIndex == 1 or iStartIndex == 2) then
                    ...
                    if data ~= nil then
                        ...
                        for i = 1, major_count do
                            if (majList[i] ~= nil) then
                                if (majList[i].leader ~= "LEADER_SPECTATOR") then
                                    local immediate_raw = (majList[i].best_tile + majList[i].best_tile_2 +
                                                              majList[i].best_tile_3 + majList[i].isBase22)
                                    local missing_amount = 13 - immediate_raw;
                                    if (immediate_raw < 13) then

                                        __Debug("Tile balancing: Need to adjust: ", majList[i].leader, "Raw Immediate:",
                                            immediate_raw, "Weak", missing_amount)
                                        Terraforming_Best(Map.GetPlot(majList[i].plotX, majList[i].plotY),
                                            missing_amount, max_best_tile_1, max_best_tile_2, avg_best_ring_1,
                                            avg_best_ring_2, majList[i].best_tile, majList[i].bestTiles1Ring1_index,
                                            majList[i].best_tile_2, majList[i].bestTiles1Ring2_index,
                                            majList[i].best_tile_3, majList[i].bestTiles1Ring3_index,
                                            majList[i].best_tile_inner, majList[i].bestTiles2Ring1_index,
                                            majList[i].best_tile_inner_2, majList[i].bestTiles2Ring2_index, 1,
                                            bHighRoll, true);
                                    else
                                        __Debug("Tile balancing: Need to adjust: ", majList[i].leader, "Raw Immediate:",
                                            immediate_raw, "Acceptable", majList[i].best_tile,
                                            majList[i].best_tile_inner)
                                    end
                                end
                            end
                        end
                                    ...
                                    if (majList[i].leader ~= "LEADER_SPECTATOR") then
                                        if majList[i].isBase22 > 1 then
                                            TerrainBuilder.SetTerrainType(
                                                Map.GetPlot(majList[i].plotX, majList[i].plotY), 3);
                                            majList[i].isBase22 = 0
                                            __Debug("Terraforming Best X: ", majList[i].plotX, "Y: ", majList[i].plotY,
                                                "Removed Plain Hills Start");
                                        end

                                        if (majList[i].best_tile < 6) and (majList[i].best_tile_inner < 6) then
                                            __Debug("Tile balancing: Need to adjust: ", majList[i].leader,
                                                "Raw Yields:", total_raw_spawn_yield,
                                                "Hasn't High Rolled - Missing Score", 6.5 -
                                                    math.max(majList[i].best_tile, majList[i].best_tile_inner))
                                            Terraforming_Best(Map.GetPlot(majList[i].plotX, majList[i].plotY), 2, 7, 7,
                                                avg_best_ring_1, avg_best_ring_2, majList[i].best_tile,
                                                majList[i].bestTiles1Ring1_index, majList[i].best_tile_2,
                                                majList[i].bestTiles1Ring2_index, majList[i].best_tile_3,
                                                majList[i].bestTiles1Ring3_index, majList[i].best_tile_inner,
                                                majList[i].bestTiles2Ring1_index, majList[i].best_tile_inner_2,
                                                majList[i].bestTiles2Ring2_index, 1, bHighRoll, true);
                                        else
                                            __Debug("Tile balancing: Need to adjust: ", majList[i].leader,
                                                "Raw Yields:", total_raw_spawn_yield, "Already High Rolled - Best",
                                                majList[i].best_tile, majList[i].best_tile_inner)
                                        end
                                    end
                                end
                            end
                        ...
                        for i = 1, major_count do
                            if (majList[i] ~= nil) then
                                if (majList[i].leader ~= "LEADER_SPECTATOR") then
                                    ...
                                    local total_raw_spawn_yield =
                                        (majList[i].best_tile + majList[i].best_tile_2 + majList[i].best_tile_3 +
                                            majList[i].best_tile_inner + majList[i].best_tile_inner_2 +
                                            majList[i].isBase22);
                                    ...
                                    if (missing_amount > 0.75) then
                                        ...
                                        __Debug("Tile balancing: Need to adjust: ", majList[i].leader, "Raw Yields:",
                                            total_raw_spawn_yield, "Missing score:", missing_amount)
                                        if (majList[i].civ == "CIVILIZATION_RUSSIA" or majList[i].civ ==
                                            "CIVILIZATION_CANADA") and majList[i].tundra_start > 3 then
                                            Terraforming_Best(Map.GetPlot(majList[i].plotX, majList[i].plotY),
                                                missing_amount, max_best_tile_1, max_best_tile_2, avg_best_ring_1,
                                                avg_best_ring_2, majList[i].best_tile, majList[i].bestTiles1Ring1_index,
                                                majList[i].best_tile_2, majList[i].bestTiles1Ring2_index,
                                                majList[i].best_tile_3, majList[i].bestTiles1Ring3_index,
                                                majList[i].best_tile_inner, majList[i].bestTiles2Ring1_index,
                                                majList[i].best_tile_inner_2, majList[i].bestTiles2Ring2_index, 1, false);
                                        elseif (majList[i].civ == "CIVILIZATION_MALI") and majList[i].desert_start > 3 then
                                            Terraforming_Best(Map.GetPlot(majList[i].plotX, majList[i].plotY),
                                                ...
                                                majList[i].best_tile_3, majList[i].bestTiles1Ring3_index,
                                                majList[i].best_tile_inner, majList[i].bestTiles2Ring1_index,
                                                majList[i].best_tile_inner_2, majList[i].bestTiles2Ring2_index, 2, false);
                                        else
                                            Terraforming_Best(Map.GetPlot(majList[i].plotX, majList[i].plotY),
                                                missing_amount, max_best_tile_1, max_best_tile_2, avg_best_ring_1,
                                                avg_best_ring_2, majList[i].best_tile, majList[i].bestTiles1Ring1_index,
                                                ...
                                                majList[i].best_tile_3, majList[i].bestTiles1Ring3_index,
                                                majList[i].best_tile_inner, majList[i].bestTiles2Ring1_index,
                                                majList[i].best_tile_inner_2, majList[i].bestTiles2Ring2_index, 0, false);
                                        end
                                    elseif (missing_amount < -1.5) then

                                        __Debug("Tile balancing: Need to adjust: ", majList[i].leader, "Raw Yields:",
                                            total_raw_spawn_yield, "Extra Yield:", -missing_amount)
                                        ...
                                        Terraforming_Best(Map.GetPlot(majList[i].plotX, majList[i].plotY),
                                            ...
                                            majList[i].best_tile_3, majList[i].bestTiles1Ring3_index,
                                            majList[i].best_tile_inner, majList[i].bestTiles2Ring1_index,
                                            majList[i].best_tile_inner_2, majList[i].bestTiles2Ring2_index, 0, bHighRoll);

                                    else

                                        __Debug("Tile balancing: No Need to adjust: ", majList[i].leader, "Raw Yields:",
                                            total_raw_spawn_yield, "Missing score:", missing_amount)
                ...
                for i = 1, major_count do
                    ...
                    if (majList[i] ~= nil) then
                        if (majList[i].leader ~= "LEADER_SPECTATOR") then
                            -- Check for freshwater
                            local wplot = Map.GetPlot(majList[i].plotX, majList[i].plotY)
                            if (wplot:IsCoastalLand() == false and wplot:IsWater() == false and wplot:IsRiver() == false and
                                wplot:IsFreshWater() == false) then
                                -- Fix No Water
                                print("Water Terraforming Start X: ", majList[i].plotX, "Start Y: ", majList[i].plotY,
                                    "Player: ", i, " ", majList[i].leader, majList[i].civ); -- put a print to catch the error in non debug mode
                                Terraforming_Water(Map.GetPlot(majList[i].plotX, majList[i].plotY), majList[i].civ);
                            end
                        end
                ...
                for i = 1, major_count do
                    ...
                    if (majList[i] == nil or majList[i].leader == "LEADER_SPECTATOR") then
                        ...
                        if (majList[i].civ == "CIVILIZATION_RUSSIA" or majList[i].civ == "CIVILIZATION_CANADA") and
                            tempEval[11] > 4 then
                        ...
                        print("BBS Script - Completed", os.date("%c"), "Player", i,
                            Locale.Lookup(PlayerConfigurations[major_table[i]]:GetPlayerName()), "Food adjustement:",
                            (tempEval[5] - majList[i].food_adjust), "Production adjustement:",
                            (tempEval[6] - majList[i].prod_adjust));
                        print("S1-S2-S3:", majList[i].best_tile, majList[i].best_tile_2, majList[i].best_tile_3,
                            "I1-I2:", majList[i].best_tile_inner, majList[i].best_tile_inner_2, "2:2 Base?",
                            majList[i].isBase22)
                    end
        ...
        for iPlotIndex = 0, Map.GetPlotCount() - 1, 1 do
            ...
            if (pPlot:GetFeatureType() == g_FEATURE_VOLCANO) then
                ...
                if iPlotTerrain ~= 2 and iPlotTerrain ~= 5 and iPlotTerrain ~= 8 and iPlotTerrain ~= 11 and iPlotTerrain ~=
                    14 then
...
function EvaluateStartingLocation(plot)
    ...
    local EvalType = {impassable, water, snow, desert, food_spawn_start, prod_spawn_start, culture_spawn_start,
                      faith_spawn_start, impassable_start, water_start, snow_start, desert_start, impassable_inner,
                      water_inner, snow_inner, desert_inner, impassable_outer, water_outer, snow_outer, desert_outer}
    ...
    for i = -1, 35 do
        ...
        if (adjacentPlot ~= nil) then
            ...
            if (i == -1) then
                ...
                if (adjacentPlot:GetFeatureType() == g_FEATURE_FLOODPLAINS or adjacentPlot:GetFeatureType() ==
                    g_FEATURE_FLOODPLAINS_PLAINS or adjacentPlot:GetFeatureType() == g_FEATURE_FLOODPLAINS_GRASSLAND) then
                    flood = flood + 1;
                end
                ...
                for row = 0, iResourcesInDB do
                    if (eResourceClassType[row] == "RESOURCECLASS_LUXURY") then
                        if (adjacentPlot:GetResourceCount() > 0) then
                            ...
                            if (adjacentPlot:GetResourceType() == 12 or adjacentPlot:GetResourceType() == 20 or
                                adjacentPlot:GetResourceType() == 21 or adjacentPlot:GetResourceType() == 49) then
                                bCulture = true;
                            elseif (adjacentPlot:GetResourceType() == 15 or adjacentPlot:GetResourceType() == 18 or
                                adjacentPlot:GetResourceType() == 23) then
                                bFaith = true;
                            end
                        end
                    end
            ...
            elseif (i > -1 and i < 6) then
               ...
               --]]
                ...
                if (adjacentPlot:GetResourceType() ~= -1) then
                    if (adjacentPlot:GetResourceType() == 41 or adjacentPlot:GetResourceType() == 46 or
                        adjacentPlot:GetResourceType() == 43) then
                ...
                if (adjacentPlot:GetResourceType() ~= -1) then
                    ...
                    if (adjacentPlot:GetResourceType() == 12 or adjacentPlot:GetResourceType() == 20 or
                        adjacentPlot:GetResourceType() == 21 or adjacentPlot:GetResourceType() == 25 or
                        adjacentPlot:GetResourceType() == 49) then
                        bCulture = true;
                    elseif (adjacentPlot:GetResourceType() == 15 or adjacentPlot:GetResourceType() == 18 or
                        adjacentPlot:GetResourceType() == 23) then
                        bFaith = true;
            ...
            elseif (i > 5 and i < 18) then
                ...
                if temp_tile > 1 then
                    temp_tile = temp_tile + adjacentPlot:GetYield(g_YIELD_PRODUCTION) * 1.5 +
                                    adjacentPlot:GetYield(g_YIELD_GOLD) * 0.25;
                elseif adjacentPlot:GetYield(g_YIELD_PRODUCTION) < 3.5 then -- not enough food to value those tiles fully
                    temp_tile = temp_tile + adjacentPlot:GetYield(g_YIELD_PRODUCTION) * 0.5 +
                                    adjacentPlot:GetYield(g_YIELD_GOLD) * 0;
                else
                    temp_tile = temp_tile + adjacentPlot:GetYield(g_YIELD_PRODUCTION) * 0.75 +
                                    adjacentPlot:GetYield(g_YIELD_GOLD) * 0;
                end
    ...
    __Debug("Evaluate Start X: ", plot:GetX(), "Evaluate Start Y: ", plot:GetY(), "Total mountain: ", impassable,
        "Total water: ", water, "Total snow: ", snow, "Total desert: ", desert, "Total hill", hill, "Immediate Food: ",
        food_spawn_start, "Immediate Prod: ", prod_spawn_start, "Immediate Culture: ", culture_spawn_start,
        "Immediate Faith: ", faith_spawn_start, "Floodplains", flood, "Best_tile", best_tile, "Best_tile_2",
        second_best_tile, "Plains Tiles", plains)
    EvalType = {impassable, water, snow, desert, food_spawn_start, prod_spawn_start, culture_spawn_start,
                faith_spawn_start, impassable_start, water_start, snow_start, desert_start, impassable_inner,
                water_inner, snow_inner, desert_inner, impassable_outer, water_outer, snow_outer, desert_outer, flood,
                hill_start, hill_inner, best_tile, second_best_tile, food_spawn_inner, prod_spawn_inner,
                best_tile_inner, second_best_tile_inner, plains, plainhills, best_tile_i, second_best_tile_i,
                best_tile_inner_i, second_best_tile_inner_i, third_best_tile, third_best_tile_i}
    return EvalType
end
    ...
    for i = start_i, end_i do
        ...
        if (adjacentPlot ~= nil) then
            ...
            if (temp_tile > cap_yield) then
                __Debug("Terraforming_Cap_Yield Start X: ", plot:GetX(), "Evaluate Start Y: ", plot:GetY(),
                    "Total Score:", temp_tile, "Exceeded Cap will remove resource", adjacentPlot:GetResourceType())
    ...
    for k = 0, 1 do
        ...
        for i = start_range, end_range, increment do
            ...
            if (adjacentPlot ~= nil) then
                ...
                -- Floodplains Only
                if (adjacentPlot:GetFeatureType() == g_FEATURE_FLOODPLAINS or adjacentPlot:GetFeatureType() ==
                    g_FEATURE_FLOODPLAINS_PLAINS or adjacentPlot:GetFeatureType() == g_FEATURE_FLOODPLAINS_GRASSLAND) and
                    adjacentPlot:GetResourceCount() < 1 then

                    rng = TerrainBuilder.GetRandomNumber(100, "test") / 100;
                    if rng < 0.75 then
                        -- Plains wheat
                        ResourceBuilder.SetResourceType(adjacentPlot, 9, 1);
                        __Debug("Food Balancing X: ", adjacentPlot:GetX(), "Food Balancing Y: ", adjacentPlot:GetY(),
                            "Added wheat on Floodplains");
                        return true;
                    end
                ...
                if (terrainType == 4 and adjacentPlot:GetResourceType() == 7) then
                    ...
                    if rng < 0.25 then
                        ...
                        __Debug("Food Balancing X: ", adjacentPlot:GetX(), "Food Balancing Y: ", adjacentPlot:GetY(),
                            "Converted Plains Hill Sheep into Grassland Hill Ship");
                        return true;
                    end
                ...
                if adjacentPlot:GetFeatureType() ~= g_FEATURE_FLOODPLAINS and adjacentPlot:GetResourceType() < 0 and
                    adjacentPlot:GetFeatureType() ~= g_FEATURE_MARSH and adjacentPlot:GetFeatureType() ~=
                    g_FEATURE_VOLCANO and adjacentPlot:GetFeatureType() ~= g_FEATURE_FLOODPLAINS_PLAINS and
                    adjacentPlot:GetFeatureType() ~= g_FEATURE_FLOODPLAINS_GRASSLAND and adjacentPlot:IsNaturalWonder() ==
                    false then
                    ...
                    if (adjacentPlot:GetFeatureType() == g_FEATURE_JUNGLE and terrainType ~= 4) then
                        ...
                        if (ResourceBuilder.CanHaveResource(adjacentPlot, 0)) then
                            ...
                            __Debug("Food Balancing X: ", adjacentPlot:GetX(), "Food Balancing Y: ",
                                adjacentPlot:GetY(), "Added: Banana");
                            return true;
                        end

                    elseif (terrainType == 3 and adjacentPlot:GetFeatureType() == -1 and adjacentPlot:GetY() >
                        gridHeight * 0.33 and adjacentPlot:GetY() < gridHeight * 0.66) then
                        -- Jungle
                        TerrainBuilder.SetFeatureType(adjacentPlot, -1)
                    ...
                    elseif ((terrainType == 6 and adjacentPlot:GetFeatureType() ~= g_FEATURE_OASIS) or
                        (terrainType == 9 and flag ~= 1)) and adjacentPlot:IsNaturalWonder() == false and flag ~= 2 then
                        -- Convert to Grassland
                        if (rng > limit_1) then
                            TerrainBuilder.SetTerrainType(adjacentPlot, 0);
                            TerrainBuilder.SetFeatureType(adjacentPlot, -1);
                            __Debug("Food Balancing X: ", adjacentPlot:GetX(), "Food Balancing Y: ",
                                adjacentPlot:GetY(), "Turned the tile to a Grassland");
                            return true;
                        end
                        rng = TerrainBuilder.GetRandomNumber(100, "test") / 100;

                    elseif ((terrainType == 4 and flag ~= 1 and rng > 0.75) or (terrainType == 7) or
                        (terrainType == 10 and flag ~= 1)) and flag ~= 2 then
                        -- Convert to Grassland
                        if (rng > limit_1) then
                            TerrainBuilder.SetTerrainType(adjacentPlot, 1);
                            TerrainBuilder.SetFeatureType(adjacentPlot, -1)
                            __Debug("Food Balancing X: ", adjacentPlot:GetX(), "Food Balancing Y: ",
                                adjacentPlot:GetY(), "Turned the tile to a Hills Grassland");
                            return true;
                        end
                    ...
                    elseif (terrainType == 0) then
                        ...
                        if (rng > 0.5) then
                            if (ResourceBuilder.CanHaveResource(adjacentPlot, 1)) then
                                ...
                                __Debug("Food Balancing X: ", adjacentPlot:GetX(), "Food Balancing Y: ",
                                    adjacentPlot:GetY(), "Added: Cattle");
                                return true;
                            end
                        else
                    ...
                    elseif (terrainType == 3 and adjacentPlot:GetFeatureType() == -1) then
                        ...
                        elseif (rng > 0.1) then
                            ...
                            __Debug("Food Balancing X: ", adjacentPlot:GetX(), "Food Balancing Y: ",
                                adjacentPlot:GetY(), "Added: Sheep on Plains Hills");
                            return true;
                            -- Maize
                        else
                            ResourceBuilder.SetResourceType(adjacentPlot, 52, 1);
                            __Debug("Food Balancing X: ", adjacentPlot:GetX(), "Food Balancing Y: ",
                                adjacentPlot:GetY(), "Added: Maize");
                            return true;
                        end
                    ...
                    elseif (terrainType == 4 and adjacentPlot:GetFeatureType() == -1 and rng < 0.75) then
                        ...
                        ResourceBuilder.SetResourceType(adjacentPlot, 9, 1);
                        __Debug("Food Balancing X: ", adjacentPlot:GetX(), "Food Balancing Y: ", adjacentPlot:GetY(),
                            "Turned Plain Hills into Plain with wheat");
                        return true;

                    elseif ((terrainType == 4 and adjacentPlot:GetFeatureType() == -1) or
                        (terrainType == 1 and adjacentPlot:GetFeatureType() == -1) or
                        (terrainType == 7 and adjacentPlot:GetFeatureType() == -1 and rng < 0.5) or
                        (terrainType == 10 and adjacentPlot:GetFeatureType() == -1)) then
                        -- sheep
                        if (ResourceBuilder.CanHaveResource(adjacentPlot, 7)) then
                            ResourceBuilder.SetResourceType(adjacentPlot, 7, 1);
                            __Debug("Food Balancing X: ", adjacentPlot:GetX(), "Food Balancing Y: ",
                                adjacentPlot:GetY(), "Added: Sheep");
                            return true;
                        end
                    ...
                    elseif (terrainType == 6 and adjacentPlot:GetFeatureType() == -1) then
                        ...
                        for j = 0, 5 do
                            ...
                            if (adjacentPlot2 ~= nil) then
                                if (adjacentPlot2:GetTerrainType() ~= 6 and adjacentPlot2:GetTerrainType() ~= 7 and
                                    adjacentPlot2:GetTerrainType() ~= 8 or adjacentPlot2:GetFeatureType() ==
                    ...
                    elseif (terrainType == 15) then
                        ...
                        if (ResourceBuilder.CanHaveResource(adjacentPlot, 5) and isSameTile(adjacentPlot, harborPlot) ==
                            false and (flag ~= 1 or (flag == 1 and rng > 0.5))) then

                            ResourceBuilder.SetResourceType(adjacentPlot, 5, 1);
                            __Debug("Food Balancing X: ", adjacentPlot:GetX(), "Food Balancing Y: ",
                                adjacentPlot:GetY(), "Added: Fish");
                            return true;
                        end

                        ------- END CHANGE  ----------

                    elseif ((terrainType == 4 and adjacentPlot:GetFeatureType() == 3) or
                        (terrainType == 1 and adjacentPlot:GetFeatureType() == 3) or
                        (terrainType == 7 and adjacentPlot:GetFeatureType() == 3) or
                        (terrainType == 10 and adjacentPlot:GetFeatureType() == 3) and count < max_unFeature) then
                    ...
                    elseif ((terrainType == 3 and adjacentPlot:GetFeatureType() == 3 and rng > 0.5)) then
                        ...
                        ResourceBuilder.SetResourceType(adjacentPlot, 7, 1);
                        __Debug("Food Balancing X: ", adjacentPlot:GetX(), "Food Balancing Y: ", adjacentPlot:GetY(),
                            "Added: Sheep on Plains Hills");
                        return true;

                        -- Add floodplains
                    elseif ((terrainType == 6 and flag == 2 and adjacentPlot:IsRiver() == true and
                        adjacentPlot:GetFeatureType() == -1)) then
                        -- Add Desert Floodplains
                        TerrainBuilder.SetFeatureType(adjacentPlot, g_FEATURE_FLOODPLAINS);
                        __Debug("Food Balancing X: ", adjacentPlot:GetX(), "Food Balancing Y: ", adjacentPlot:GetY(),
                            "Turned the tile to a Desert Floodplains");
                        return true;

                        -- Mountains to Hills
                    elseif (flag ~= 2 and flag ~= 1 and
                        ((terrainType == 2 and flag ~= 3) or (terrainType == 5 and flag ~= 3) or
                            (terrainType == 8 and flag ~= 3 and flag ~= 2) or
                            (terrainType == 11 and flag ~= 3 and flag ~= 1) or
                            (terrainType == 14 and flag ~= 3 and flag ~= 1))) then
                        ...
                        if rng > 0.90 and terrainType == 2 then
                            ...
                            __Debug("Food Balancing X: ", adjacentPlot:GetX(), "Food Balancing Y: ",
                                adjacentPlot:GetY(), "Turned the Grass Mountain to a Flat land with stones");
                            return true;
                        elseif rng > 0.70 then
                            TerrainBuilder.SetTerrainType(adjacentPlot, terrainType - 1);
                            __Debug("Food Balancing X: ", adjacentPlot:GetX(), "Food Balancing Y: ",
                                adjacentPlot:GetY(), "Turned the Mountain to a Hill");
                            return true
                        end
    ...
    for k = 0, 1 do
        ...
        for i = start_range, end_range, increment do
            ...
            if (adjacentPlot ~= nil) then
                ...
                if (adjacentPlot:IsNaturalWonder() == false and adjacentPlot:GetFeatureType() ~= g_FEATURE_FLOODPLAINS and
                    adjacentPlot:GetFeatureType() ~= g_FEATURE_FLOODPLAINS_PLAINS and adjacentPlot:GetFeatureType() ~=
                    g_FEATURE_FLOODPLAINS_GRASSLAND and adjacentPlot:GetFeatureType() ~= g_FEATURE_VOLCANO) then

                    -- No Resource
                    if (adjacentPlot:GetResourceType() < 0) then

                        -- No feature
                        if ((adjacentPlot:GetFeatureType() == -1) and (adjacentPlot:IsImpassable() == false) and
                            (adjacentPlot:IsWater() == false) and (adjacentPlot:GetTerrainType() ~= 6) and
                            (adjacentPlot:GetTerrainType() ~= 7) and (adjacentPlot:GetTerrainType() ~= 12) and
                            (adjacentPlot:GetTerrainType() ~= 13)) then
                            -- Wood
                            -- __Debug("Prod balancing: Wood");
                            if rng > 0.05 then
                                TerrainBuilder.SetFeatureType(adjacentPlot, 3);
                                ResourceBuilder.SetResourceType(adjacentPlot, -1);
                                __Debug("Prod Balancing X: ", adjacentPlot:GetX(), "Prod Balancing Y: ",
                                    adjacentPlot:GetY(), "Added: Wood");
                                return true;
                            end
                        end
                        ...
                        if (terrainType == 0 or terrainType == 1) and rng > 0.90 then
                            ...
                            TerrainBuilder.SetFeatureType(adjacentPlot, -1);
                            __Debug("Food Balancing X: ", adjacentPlot:GetX(), "Food Balancing Y: ",
                                adjacentPlot:GetY(), "Flat land with stones");
                            return true
                        end
                        ...
                        if (terrainType == 0 and adjacentPlot:GetFeatureType() ~= g_FEATURE_MARSH) and rng > 0.50 then
                            ...
                            if (rng > limit_1) then
                                TerrainBuilder.SetTerrainType(adjacentPlot, 1);
                                __Debug("Prod Balancing X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                                    "Turned the tile to a Grassland Hill");
                                return true;
                            end
                        end
                        ...
                        if (terrainType == 3 and adjacentPlot:GetFeatureType() ~= g_FEATURE_MARSH) and rng > 0.50 then
                            ...
                            if (rng > limit_1) then
                                TerrainBuilder.SetTerrainType(adjacentPlot, 4);
                                __Debug("Prod Balancing X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                                    "Turned the tile to a Plain Hill");
                                return true;
                            end
                        end
                        ...
                        if (terrainType == 6 and adjacentPlot:GetFeatureType() ~= g_FEATURE_OASIS) then
                            ...
                            if (rng > limit_1) then
                                TerrainBuilder.SetTerrainType(adjacentPlot, 7);
                                __Debug("Prod Balancing X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                                    "Turned the tile to a Desert Hill");
                                return true;
                            end
                        end

                        if ((terrainType == 2 and flag ~= 3) or (terrainType == 5 and flag ~= 3) or
                            (terrainType == 8 and flag ~= 3) or (terrainType == 11 and flag ~= 3) or
                            (terrainType == 14 and flag ~= 3)) then
                            ...
                            if (rng > limit_1) then
                                ...
                                __Debug("Prod Balancing X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                                    "Turned the Mountain tile to a Hill");
                                return true;
                            end
                        end
                        ...
                        if (terrainType == 9 and adjacentPlot:GetFeatureType() ~= g_FEATURE_MARSH) then
                            ...
                            if (rng > limit_1) then
                                ...
                                __Debug("Prod Balancing X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                                    "Turned the tile to a Tundra Hill");
                                return true;
                            end
                        end
                        ...
                        if ((terrainType == 7) or (terrainType == 10)) then
                            ...
                            if (ResourceBuilder.CanHaveResource(adjacentPlot, 2)) then
                                ...
                                TerrainBuilder.SetFeatureType(adjacentPlot, -1);
                                __Debug("Prod Balancing X: ", adjacentPlot:GetX(), "Prod Balancing Y: ",
                                    adjacentPlot:GetY(), "Added: Copper");
                                return true;
                            end
                        end
                    ...
                    -- Water
                    if (terrainType == 15 and adjacentPlot:GetFeatureType() == -1 and
                        (adjacentPlot:GetResourceCount() < 1 or adjacentPlot:GetResourceType() == 5)) then
                        bWater = true;
                        for j = 0, 5 do
                            if (adjacentPlot:GetX() >= 0 and adjacentPlot:GetY() < gridHeight) then
                                if (Map.GetAdjacentPlot(adjacentPlot:GetX(), adjacentPlot:GetY(), j) ~= nil) then
                                    ...
                                    if (Map.GetAdjacentPlot(adjacentPlot:GetX(), adjacentPlot:GetY(), j):IsWater() ==
                                        false) then
                                        bWater = false;
                                    end
                                else
                        ...
                        if (rng > limit_1 and bWater == true) then
                            ...
                            local list = getRing(adjacentPlot:GetX(), adjacentPlot:GetY(), 1, mapXSize, mapXSize,
                                mapIsRoundWestEast);
                            for _, element in ipairs(list) do
                                local x = element[1];
                                local y = element[2];
                    ...
                    elseif (terrainType == 15 and adjacentPlot:GetFeatureType() == -1 and adjacentPlot:IsFreshWater() ==
                        false and (adjacentPlot:GetResourceCount() < 1 or adjacentPlot:GetResourceType() == 5)) then
                    ...
                    elseif (terrainType == 15 and adjacentPlot:GetFeatureType() == g_FEATURE_REEF and
                        adjacentPlot:GetResourceType() == -1) then
    ...
    for k = 0, 1 do
        ...
        for i = start_range, end_range, increment do
            ...
            if (adjacentPlot ~= nil) then
                if adjacentPlot:IsNaturalWonder() == false and adjacentPlot:GetFeatureType() ~= g_FEATURE_FLOODPLAINS and
                    adjacentPlot:GetFeatureType() ~= g_FEATURE_FLOODPLAINS_PLAINS and adjacentPlot:GetFeatureType() ~=
                    g_FEATURE_FLOODPLAINS_GRASSLAND then
                    terrainType = adjacentPlot:GetTerrainType();
                    rng = TerrainBuilder.GetRandomNumber(100, "test") / 100;
                    if (terrainType == 0 and adjacentPlot:GetResourceType() == -1 and adjacentPlot:GetFeatureType() ~=
                        g_FEATURE_MARSH) then
                        if (rng > limit) then
                            TerrainBuilder.SetTerrainType(adjacentPlot, 1);
                            __Debug("Prod Balancing X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                                "Turned the tile to a Grassland Hill");
                    ...
                    elseif (terrainType == 3 and adjacentPlot:GetResourceType() == -1 and adjacentPlot:GetFeatureType() ~=
                        g_FEATURE_MARSH) then
                        if (rng > limit) then
                            TerrainBuilder.SetTerrainType(adjacentPlot, 4);
                            __Debug("Prod Balancing X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                    ...
                    elseif (terrainType == 6 and adjacentPlot:GetResourceType() == -1 and adjacentPlot:GetFeatureType() ~=
                        g_FEATURE_MARSH and adjacentPlot:GetFeatureType() ~= g_FEATURE_OASIS) then
                        if (rng > limit) then
                            TerrainBuilder.SetTerrainType(adjacentPlot, 7);
                            __Debug("Prod Balancing X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                                "Turned the tile to a Desert Hill");
                    ...
                    elseif (terrainType == 9 and adjacentPlot:GetResourceType() == -1 and adjacentPlot:GetFeatureType() ~=
                        g_FEATURE_MARSH) then
                        if (rng > limit) then
                            TerrainBuilder.SetTerrainType(adjacentPlot, 10);
                            __Debug("Prod Balancing X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                    ...
                    elseif (adjacentPlot:GetResourceType() == -1 and adjacentPlot:GetFeatureType() == g_FEATURE_MARSH and
                        (terrainType == 0 or terrainType == 3)) then
                        if (rng > limit * 2) then
                            TerrainBuilder.SetTerrainType(adjacentPlot, terrainType + 1);
                            TerrainBuilder.SetFeatureType(adjacentPlot, -1);
                            __Debug("Prod Balancing X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                                "Turned the Marsh tile to a Hill");
                            return true;
                        end
                    end
...
function Terraforming_Nuke_Mountain(plot)
    ...
    for i = -1, 17 do
        ...
        if (adjacentPlot ~= nil) then
            if ((adjacentPlot:GetTerrainType() == 2 or adjacentPlot:GetTerrainType() == 5 or
                adjacentPlot:GetTerrainType() == 8 or adjacentPlot:GetTerrainType() == 11 or
                adjacentPlot:GetTerrainType() == 14) and adjacentPlot:GetFeatureType() ~= g_FEATURE_VOLCANO) and
                adjacentPlot:IsNaturalWonder() == false then
                local rng = TerrainBuilder.GetRandomNumber(100, "test") / 100;
                if (rng > limit) then
                    __Debug("Nuked Mountain X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                        "Replaced a Mountain by a Hill");
...
function Terraforming_Mountain(plot, flag)
    ...
    if flag == 3 then
        for i = 1, 60 do
            if (GetAdjacentTiles(plot, i) ~= nil) then
                rng = TerrainBuilder.GetRandomNumber(100, "test") / 100
                adjacentPlot = GetAdjacentTiles(plot, i)
                if (adjacentPlot:IsImpassable() == false and adjacentPlot:IsWater() == false and
                    adjacentPlot:IsNaturalWonder() == false and adjacentPlot:GetResourceCount() < 1 and
                    adjacentPlot:GetFeatureType() ~= g_FEATURE_FLOODPLAINS and adjacentPlot:GetFeatureType() ~=
                    g_FEATURE_FLOODPLAINS_GRASSLAND and adjacentPlot:GetFeatureType() ~= g_FEATURE_FLOODPLAINS_PLAINS and
                    rng < 0.30 and count < 6) then
                    ...
                    count = count + 1
                    __Debug("Terraforming_Mountain X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                        "Place a Mountain (Inca)");
                end
            end
        end
    end
    ...
    for i = 0, 5 do
        if (GetAdjacentTiles(plot, i) ~= nil) then
            if (GetAdjacentTiles(plot, i):IsImpassable() == true) and GetAdjacentTiles(plot, i):IsNaturalWonder() ==
                false then
                -- immediate wall
                __Debug("Terraforming_Mountain X: ", GetAdjacentTiles(plot, i):GetX(), "Y: ",
                    GetAdjacentTiles(plot, i):GetY(), "Analysing the plot");
                if (i == 0) then
                    if (GetAdjacentTiles(plot, 5) ~= nil and GetAdjacentTiles(plot, i + 1) ~= nil) then
                        if (GetAdjacentTiles(plot, 5):IsImpassable() == true and
                            GetAdjacentTiles(plot, i + 1):IsImpassable() == true) then
                            -- Walled-in is there actual terrain on the other side ?
                            if (GetAdjacentTiles(plot, 5 * i + 60) ~= nil) then
                                if (GetAdjacentTiles(plot, 5 * i + 60):IsImpassable() == false and
                                    GetAdjacentTiles(plot, 5 * i + 60):IsWater() == false) then
                                    -- Ok there is land let measure the distance to dig through
                                    if (GetAdjacentTiles(plot, 2 * i + 6) ~= nil and GetAdjacentTiles(plot, 3 * i + 18) ~=
                                        nil and GetAdjacentTiles(plot, 4 * i + 36) ~= nil) then
                                        ...
                                        __Debug("Terraforming_Mountain X: ", GetAdjacentTiles(plot, i):GetX(), "Y: ",
                                            GetAdjacentTiles(plot, i):GetY(), "Distance to dig is", distance);
                                    ...
                                    __Debug("Terraforming_Mountain X: ", GetAdjacentTiles(plot, i):GetX(), "Y: ",
                                        GetAdjacentTiles(plot, i):GetY(), "No good Terrain on the other side");
                                end
                            else
                                __Debug("Terraforming_Mountain X: ", GetAdjacentTiles(plot, i):GetX(), "Y: ",
                                    GetAdjacentTiles(plot, i):GetY(), "No Terrain on the other side");
                            end
                        else
                            __Debug("Terraforming_Mountain X: ", GetAdjacentTiles(plot, i):GetX(), "Y: ",
                                GetAdjacentTiles(plot, i):GetY(), "Can move around the Mountain");
                        end
                    end
                elseif (i > 0 and i < 5) then
                    if (GetAdjacentTiles(plot, i - 1) ~= nil and GetAdjacentTiles(plot, i + 1) ~= nil) then
                        if (GetAdjacentTiles(plot, i - 1):IsImpassable() == true and
                            GetAdjacentTiles(plot, i + 1):IsImpassable() == true) then
                            -- Walled-in is there actual terrain on the other side ?
                            if (GetAdjacentTiles(plot, 5 * i + 60) ~= nil) then
                                if (GetAdjacentTiles(plot, 5 * i + 60):IsImpassable() == false and
                ...
                elseif (i == 5) then
                    if (GetAdjacentTiles(plot, i - 1) ~= nil and GetAdjacentTiles(plot, 0) ~= nil) then
                        if (GetAdjacentTiles(plot, i - 1):IsImpassable() == true and
                            GetAdjacentTiles(plot, 0):IsImpassable() == true) then
                            -- Walled-in is there actual terrain on the other side ?
                            if (GetAdjacentTiles(plot, 5 * i + 60) ~= nil) then
                                if (GetAdjacentTiles(plot, 5 * i + 60):IsImpassable() == false and
                                    GetAdjacentTiles(plot, 5 * i + 60):IsWater() == false) then
                ...
                if (GetAdjacentTiles(plot, 2 * i + 6) ~= nil) then
                    __Debug("Terraforming_Mountain X: ", GetAdjacentTiles(plot, 2 * i + 6):GetX(), "Y: ",
                        GetAdjacentTiles(plot, 2 * i + 6):GetY(), "Analysing the plot");
                    if (GetAdjacentTiles(plot, 2 * i + 6):IsImpassable() == true) then
                        if (i == 0) then
                            if (GetAdjacentTiles(plot, 17) ~= nil and GetAdjacentTiles(plot, 2 * i + 6 + 1) ~= nil) then
                                if (GetAdjacentTiles(plot, 17):IsImpassable() == true and
                                    GetAdjacentTiles(plot, 2 * i + 1 + 6):IsImpassable() == true) then
                                    -- Walled-in is there actual terrain on the other side ?
                                    if (GetAdjacentTiles(plot, 5 * i + 60) ~= nil) then
                                        if (GetAdjacentTiles(plot, 5 * i + 60):IsImpassable() == false and
                                            GetAdjacentTiles(plot, 5 * i + 60):IsWater() == false) then
                                            -- Ok there is land let measure the distance to dig through
                                            if (GetAdjacentTiles(plot, 3 * i + 18) ~= nil and
                                                GetAdjacentTiles(plot, 4 * i + 36) ~= nil) then
                                                if (GetAdjacentTiles(plot, 3 * i + 18):IsImpassable() == true) then
                                                    distance = 1;
                                                else
                                                    distance = 0;
                                                ...
                                                __Debug("Terraforming_Mountain X: ", GetAdjacentTiles(plot, i):GetX(),
                                                    "Y: ", GetAdjacentTiles(plot, i):GetY(), "Distance to dig is",
                                                    distance)
                                                if (distance < min_distance) then
                                                    min_distance = distance;
                                                    minimal_effort_i = i;
                                            ...
                                            __Debug("Terraforming_Mountain X: ", GetAdjacentTiles(plot, i):GetX(),
                                                "Y: ", GetAdjacentTiles(plot, i):GetY(),
                                                "No good Terrain on the other side");
                                    ...
                                    __Debug("Terraforming_Mountain X: ", GetAdjacentTiles(plot, 2 * i + 6):GetX(),
                                        "Y: ", GetAdjacentTiles(plot, 2 * i + 6):GetY(), "Can move around the Mountain");
                                end
                            end
                        elseif (i > 0) then
                            if (GetAdjacentTiles(plot, 2 * i + 6 - 1) ~= nil and GetAdjacentTiles(plot, 2 * i + 1 + 6) ~=
                                nil) then
                                if (GetAdjacentTiles(plot, 2 * i + 6 - 1):IsImpassable() == true and
                                    GetAdjacentTiles(plot, 2 * i + 1 + 6):IsImpassable() == true) then
                                    -- Walled-in is there actual terrain on the other side ?
                                    if (GetAdjacentTiles(plot, 5 * i + 60) ~= nil) then
                                        if (GetAdjacentTiles(plot, 5 * i + 60):IsImpassable() == false and
    ...
    if (minimal_effort_i ~= nil) then
        __Debug("Terraforming_Mountain X: ", GetAdjacentTiles(plot, minimal_effort_i):GetX(), "Y: ",
            GetAdjacentTiles(plot, minimal_effort_i):GetY(), "Digging an openning");
        if (GetAdjacentTiles(plot, minimal_effort_i) ~= nil and GetAdjacentTiles(plot, 2 * minimal_effort_i + 6) ~= nil and
            GetAdjacentTiles(plot, 3 * minimal_effort_i + 18) ~= nil and
            GetAdjacentTiles(plot, 4 * minimal_effort_i + 36) ~= nil) then
            ...
            if (adjacentPlot:IsImpassable() == true and adjacentPlot:GetFeatureType() ~= g_FEATURE_VOLCANO) then
                ...
                if adjacentPlot:GetTerrainType() == 10 or adjacentPlot:GetTerrainType() == 13 or
                    adjacentPlot:GetTerrainType() == 7 then
                    -- TerrainBuilder.SetTerrainType(adjacentPlot,4)
            ...
            if (adjacentPlot2:IsImpassable() == true and adjacentPlot2:GetFeatureType() ~= g_FEATURE_VOLCANO) then
                ...
                if adjacentPlot2:GetTerrainType() == 10 or adjacentPlot2:GetTerrainType() == 13 or
                    adjacentPlot2:GetTerrainType() == 7 then
                ...
                __Debug("Terraforming_Mountain X: ", adjacentPlot2:GetX(), "Y: ", adjacentPlot2:GetY(),
                    "turn Mountain into a Hill");
            end
            if (adjacentPlot3:IsImpassable() == true and adjacentPlot3:GetFeatureType() ~= g_FEATURE_VOLCANO) then
                ...
                if adjacentPlot3:GetTerrainType() == 10 or adjacentPlot3:GetTerrainType() == 13 or
                    adjacentPlot3:GetTerrainType() == 7 then
                ...
                __Debug("Terraforming_Mountain X: ", adjacentPlot3:GetX(), "Y: ", adjacentPlot3:GetY(),
                    "turn Mountain into a Hill");
            end
            if (adjacentPlot4:IsImpassable() == true and adjacentPlot4:GetFeatureType() ~= g_FEATURE_VOLCANO) then
                ...
                if adjacentPlot4:GetTerrainType() == 10 or adjacentPlot4:GetTerrainType() == 13 or
                    adjacentPlot4:GetTerrainType() == 7 then
                ...
                __Debug("Terraforming_Mountain X: ", adjacentPlot4:GetX(), "Y: ", adjacentPlot4:GetY(),
                    "turn Mountain into a Hill");
            end
...
function Terraforming_Best_Refresh(majList, major_count, step, bHighRoll)
    ...
    for i = 1, major_count do
        if (majList[i] == nil or majList[i].leader == "LEADER_SPECTATOR") then
            ...
            if (majList[i].civ == "CIVILIZATION_RUSSIA" or majList[i].civ == "CIVILIZATION_CANADA") and _tempEval[11] >
                4 then
                -- Russia/Canada on Tundra
                --	Ring 1
            ...
            __Debug(majList[i].civ, "S1-S2-S3-I1-I2:", majList[i].best_tile, "(", majList[i].bestTiles1Ring1_index, ")",
                majList[i].best_tile_2, "(", majList[i].bestTiles1Ring2_index, ")", majList[i].best_tile_3, "(",
                majList[i].bestTiles1Ring3_index, ")", majList[i].best_tile_inner, "(",
                majList[i].bestTiles2Ring1_index, ")", majList[i].best_tile_inner_2, "(",
                majList[i].bestTiles2Ring2_index, ") - 2:2 Base?", majList[i].isBase22)
            ...
            if (majList[i].best_tile + majList[i].best_tile_2 + majList[i].best_tile_3 + majList[i].best_tile_inner +
                majList[i].best_tile_inner_2 + majList[i].isBase22) > max_best_tile_3 then
                max_best_tile_3 = (majList[i].best_tile + majList[i].best_tile_2 + majList[i].best_tile_3 +
                                      majList[i].best_tile_inner + majList[i].best_tile_inner_2 + majList[i].isBase22)
                best_civ_3 = majList[i].leader
            end
            -- Best Score
            if (majList[i].best_tile + majList[i].best_tile_2 + majList[i].best_tile_3 + majList[i].best_tile_inner *
                0.9 + majList[i].best_tile_inner_2 * 0.9 + majList[i].isBase22) > max_best_tile_4 then
                max_best_tile_4 = (majList[i].best_tile + majList[i].best_tile_2 + majList[i].best_tile_3 +
                                      majList[i].best_tile_inner * 0.9 + majList[i].best_tile_inner_2 * 0.9 +
                                      majList[i].isBase22)
                best_civ_4 = majList[i].leader
            end
            avg_best_ring_1 = avg_best_ring_1 + majList[i].best_tile + majList[i].best_tile_2 + majList[i].best_tile_3 +
                                  majList[i].isBase22;
...
function Terraforming_Best(plot, missing_amount, best_1ring, best_2ring, avg_ring1, avg_ring2, onering1_yield,
    onering1_index, onering2_yield, onering2_index, onering3_yield, onering3_index, tworing1_yield, tworing1_index,
    tworing2_yield, tworing2_index, flag, highroll, highroll_round)
    -- flag = 0 normal
    -- flag = 1 tundra civ
    -- flag = 2 desert civ
    -- flag = 3 mountain civ
    ...
    target_tiles[0] = {
        yield = onering3_yield,
        index = onering3_index,
        isValid = false
    };
    target_tiles[1] = {
        yield = onering2_yield,
        index = onering2_index,
        isValid = false
    };
    target_tiles[2] = {
        yield = onering1_yield,
        index = onering1_index,
        isValid = false
    };
    target_tiles[3] = {
        yield = tworing2_yield,
        index = tworing2_index,
        isValid = false
    };
    target_tiles[4] = {
        yield = tworing1_yield,
        index = tworing1_index,
        isValid = false
    };
    ...
    for i = 0, 4 do
        ...
        if (target_tiles[i].yield < 5.5 or highroll == false or high_roll_count == 1) then
            if (pPlot:GetResourceCount() < 1 or (pPlot:GetResourceCount() > 0 and pPlot:GetResourceType() < 10)) then
                ...
                __Debug("Terraforming Best X: ", pPlot:GetX(), "Y: ", pPlot:GetY(), "Index", target_tiles[i].index,
                    "Not Valid - Luxuries or Strategic");
            ...
            __Debug("Terraforming Best X: ", pPlot:GetX(), "Y: ", pPlot:GetY(), "Index", target_tiles[i].index,
                "Not Valid - High Roll", target_tiles[i].yield);
            high_roll_count = 1;
        end
    end

    table.sort(target_tiles, function(a, b)
    ...
    if valid_count < 5 then
        ...
        for i = 0, 17 do
            ...
            adjacentPlot = GetAdjacentTiles(plot, i);
            if (adjacentPlot:GetResourceCount() < 1 or
                (adjacentPlot:GetResourceCount() > 0 and adjacentPlot:GetResourceType() < 10)) then
                ...
                if alreadyListed == false then
                    for j = 0, 4 do
                        if target_tiles[j].isValid == false then
                            ...
                            valid_count = valid_count + 1
                            __Debug("Terraforming Best: Not Enough Valid Plots - Add Plot X", adjacentPlot:GetX(), "Y:",
                                adjacentPlot:GetY());
                            break
                        end
                    end
    ...
    for i = 0, 2 do
        ...
        if target_tiles[i].isValid == true then
            ...
            __Debug("Terraforming Best i: ", i, "target_yield: ", target_yield, "target_tiles[i].yield",
                target_tiles[i].yield, "max_yield ", max_yield);

            -- Grassland
            if (target_plot_1:GetTerrainType() == 0 or target_plot_1:GetTerrainType() == 1 or
                (target_plot_1:GetTerrainType() == 2 and flag ~= 3 and target_plot_1:GetFeatureType() ~=
                ...
                if (target_plot_1:GetFeatureType() ~= g_FEATURE_FLOODPLAINS_GRASSLAND) then
                    ...
                    if (target_yield >= 5.75) then
                        ...
                        __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                            "Added 2/3 Forested Deer Grassland Hill");
                        placed_yield = 6.5
                    ...
                    elseif (target_yield < 5.75 and target_yield > 4.75) then
                        ...
                        if (rng >= 0.75) then
                            ...
                            ResourceBuilder.SetResourceType(target_plot_1, 4, 1)
                            __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                                "Added 2/2 Grassland Forest with Deers");
                            placed_yield = 5
                        ...
                        elseif (rng >= 0.45 and rng < 0.75) then
                            ...
                            TerrainBuilder.SetFeatureType(target_plot_1, 3);
                            __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                                "Added 2/2 Forested Grassland Hill");
                            placed_yield = 5
                        ...
                        elseif (rng >= 0 and rng < 0.45) then
                            ...
                            __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                                "Added 2/2 Stone Grassland Hill");
                            placed_yield = 5
                        ...
                        if (rng >= 0.5) then
                            ...
                            __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                                "Added 3/1 Sheep Grassland Hill");
                            placed_yield = 4.5
                            ...
                            __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                                "Added 2/1/2 Copper Grassland Hill");
                            placed_yield = 4.0
                        end
                    ...
                    if (target_plot_1:GetResourceCount() < 1) then
                        ...
                        __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                            "Added 3/0 Rice Grassland Floodplains");
                        placed_yield = 3
                    end
                ...
                -- Plains
            elseif (target_plot_1:GetTerrainType() == 3 or target_plot_1:GetTerrainType() == 4 or
                (target_plot_1:GetTerrainType() == 5 and flag ~= 3 and target_plot_1:GetFeatureType() ~=
                    g_FEATURE_VOLCANO)) then
                ...
                if (target_plot_1:GetFeatureType() ~= g_FEATURE_FLOODPLAINS_PLAINS) then
                    ...
                    if (target_yield >= 5.75) then
                        ...
                        if (rng >= 0.9) then
                            ...
                            ResourceBuilder.SetResourceType(target_plot_1, 4, 1)
                            __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                                "Added 1/4 Forested Plain Hill with Deer");
                            placed_yield = 6
                        ...
                        elseif (rng >= 0 and rng < 0.90) then
                            ...
                            if (target_plot_1:GetY() > gridHeight * 0.25 and target_plot_1:GetY() < gridHeight * 0.75) then
                                ...
                                __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                                    "Added 3/2 Jungle Plain Hill with Banana");
                                placed_yield = 6.5
                            ...
                            elseif rng > 0.75 then
                                ...
                                ResourceBuilder.SetResourceType(target_plot_1, 7, 1)
                                __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                                    "Added 2/2 Plain Hill with Sheep");
                                placed_yield = 5
                    ...
                    elseif (target_yield < 5.75 and target_yield > 4.75) then
                        ...
                        if (target_plot_1:GetY() > gridHeight * 0.33 and target_plot_1:GetY() < gridHeight * 0.66) then
                            ...
                            TerrainBuilder.SetFeatureType(target_plot_1, 2);
                            __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                                "Added 3/1 Jungle Plain with Banana");
                            placed_yield = 4.5
                        else
                            ...
                            if (rng >= 0.5) then
                                ...
                                ResourceBuilder.SetResourceType(target_plot_1, 4, 1);
                                __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                                    "Added 1/3 Forested Plain with Deer");
                                placed_yield = 4.0
                            else
                                ...
                                ResourceBuilder.SetResourceType(target_plot_1, -1);
                                __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                                    "Added 1/3 Plain hill with forest");
                                placed_yield = 4.0
                            end
                    ...
                    if (target_plot_1:GetResourceCount() < 1 and target_plot_1:GetFeatureType() ~= g_FEATURE_OASIS) then
                        ...
                        __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                            "Added 3/0 Wheat Plains Floodplains");
                    end
                    placed_yield = 3
                ...
                -- Desert
            elseif (target_plot_1:GetTerrainType() == 6 or target_plot_1:GetTerrainType() == 7 or
                (target_plot_1:GetTerrainType() == 8 and flag ~= 3 and target_plot_1:GetFeatureType() ~=
                    g_FEATURE_VOLCANO)) then

                if (target_plot_1:GetFeatureType() ~= g_FEATURE_FLOODPLAINS and target_plot_1:GetFeatureType() ~=
                    g_FEATURE_OASIS) then
                    ...
                    elseif (rng >= 0.0 and rng < 0.5) then
                        ...
                        ResourceBuilder.SetResourceType(target_plot_1, 2, 1)
                        __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                            "Added 1/0/2 Copper Desert Hill");
                        placed_yield = 2
                        if flag == 2 then
                            placed_yield = placed_yield + 1
                        end
                    ...
                    if (target_plot_1:GetResourceCount() < 1 and target_plot_1:GetFeatureType() ~= g_FEATURE_OASIS) then
                        ResourceBuilder.SetResourceType(target_plot_1, 9, 1)
                        __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                            "Added 2/0 Wheat Desert Floodplains");
                        placed_yield = 2
                    end
                end
                -- Tundra
            elseif (target_plot_1:GetTerrainType() == 9 or target_plot_1:GetTerrainType() == 10 or
                (target_plot_1:GetTerrainType() == 11 and flag ~= 3 and target_plot_1:GetFeatureType() ~=
                    g_FEATURE_VOLCANO)) then
                ...
                if (target_plot_1:GetFeatureType() ~= g_FEATURE_MARSH) then
                    ...
                    elseif (rng >= 0.5 and rng < 0.75) then
                        ...
                        ResourceBuilder.SetResourceType(target_plot_1, 2, 1)
                        __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                            "Added 1/1/2 Copper Tundra Hill");
                        placed_yield = 3.5
                        if flag == 1 then
                            placed_yield = 4.5
                        end
                ...
                __Debug("Terraforming Best Index: ", target_tiles[i].index, "X: ", pPlot:GetY(), "Y: ", pPlot:GetY(),
                    "Index", target_tiles[i].index, "Couldn't Place Any Additional Yield");
            end

            local added_yield = placed_yield - target_tiles[i].yield
            __Debug("Terraforming Best Original Yield: ", target_tiles[i].yield, "Placed Yield ", placed_yield,
                "Remaining Before", remaining_amount, "After", remaining_amount - added_yield, "Index",
                target_tiles[i].index);
    ...
    for i = 0, 2 do
        ...
        if target_tiles[i].isValid == true then
                    ...
                    if (target_plot_1:GetFeatureType() ~= g_FEATURE_FLOODPLAINS_GRASSLAND) then
                        ...
                        if (target_yield < 5.75 and target_yield > 4.75) then
                            ...
                            if (rng >= 0.70) then
                                ...
                                ResourceBuilder.SetResourceType(target_plot_1, 4, 1)
                                __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                                    "Nerfed to 2/2 Grassland Forest with Deers");
                                placed_yield = 5
                            ...
                            elseif (rng >= 0.35 and rng < 0.70) then
                                ...
                                TerrainBuilder.SetFeatureType(target_plot_1, 3);
                                __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                                    "Nerfed to 2/2 Forested Grassland Hill");
                                placed_yield = 5
                                ...
                                ResourceBuilder.SetResourceType(target_plot_1, 8, 1)
                                __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                                    "Nerfed to 2/2 Stone Grassland Hill");
                                placed_yield = 5
                        ...
                        elseif target_yield > 4.25 then -- yield below 4.75
                            ...
                            if (rng >= 0.0 and rng < 0.80) then
                                ...
                                ResourceBuilder.SetResourceType(target_plot_1, 7, 1)
                                __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                                    "Nerfed to 3/1 Sheep Grassland Hill");
                                placed_yield = 4.5
                                ...
                                ResourceBuilder.SetResourceType(target_plot_1, 2, 1)
                                __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                                    "Nerfed to 2/1/2 Copper Grassland Hill");
                                placed_yield = 4
                            end
                        ...
                        if (target_plot_1:GetResourceCount() < 1) then
                            ResourceBuilder.SetResourceType(target_plot_1, 6, 1)
                            __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                                "Nerfed to 3/0 Rice Grassland Floodplains");
                            placed_yield = 3
                        end
                    ...
                    if (target_plot_1:GetFeatureType() ~= g_FEATURE_FLOODPLAINS_PLAINS) then
                        ...
                        if (target_yield < 5.25 and target_yield > 4.75) then
                            ...
                            if (rng >= 0.50) then
                                ...
                                ResourceBuilder.SetResourceType(target_plot_1, 7, 1);
                                __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                                    "Nerfed to 2/2 Sheep Plain Hill");
                                placed_yield = 5

                            else

                                if (target_plot_1:GetY() > gridHeight * 0.33 and target_plot_1:GetY() < gridHeight *
                                    0.66) then
                                    -- Jungle Plain Hill
                                    TerrainBuilder.SetTerrainType(target_plot_1, 4);
                                    TerrainBuilder.SetFeatureType(target_plot_1, -1);
                                    ...
                                    TerrainBuilder.SetFeatureType(target_plot_1, 2);
                                    __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                                        "Nerfed to 2/2 Jungle Plain Hill");
                                    placed_yield = 5
                                else
                                    -- Hill with Sheep
                                    TerrainBuilder.SetTerrainType(target_plot_1, 4);
                        ...
                        elseif target_yield > 4.25 then -- yield < 4.75
                            ...
                            if (target_plot_1:GetY() > gridHeight * 0.33 and target_plot_1:GetY() < gridHeight * 0.66) then
                                ...
                                TerrainBuilder.SetFeatureType(target_plot_1, 2);
                                __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                                    "Nerfed to 3/1 Jungle Plain with Banana");
                                placed_yield = 4.5
                            else
                                ...
                                if (rng >= 0.50) then
                                    ...
                                    ResourceBuilder.SetResourceType(target_plot_1, 4, 1);
                                    __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                                        "Nerfed to 1/3 Forested Plain with Deer");
                                    placed_yield = 4
                                else
                                    -- Forested Plain hill
                                    TerrainBuilder.SetTerrainType(target_plot_1, 4);
                                    ...
                                    ResourceBuilder.SetResourceType(target_plot_1, -1);
                                    __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                                        "Nerfed to 1/3 Forested Plain Hill");
                                    placed_yield = 4
                                end
                        ...
                        if (target_plot_1:GetResourceCount() < 1 and target_plot_1:GetFeatureType() ~= g_FEATURE_OASIS) then
                            ResourceBuilder.SetResourceType(target_plot_1, 9, 1)
                            __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                                "Nerfed to 3/0 Wheat Plains Floodplains");
                        end
                        placed_yield = 3
                ...
                elseif (target_plot_1:GetTerrainType() == 6 or target_plot_1:GetTerrainType() == 7 or
                    ...
                    if (target_plot_1:GetFeatureType() ~= g_FEATURE_FLOODPLAINS and target_plot_1:GetFeatureType() ~=
                        ...
                        if (rng >= 0.5) then
                            ...
                            ResourceBuilder.SetResourceType(target_plot_1, 7, 1);
                            __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                                "Nerfed to 1/1 Sheep Desert Hill");
                            placed_yield = 2.5
                            if flag == 2 then
                                placed_yield = placed_yield + 1
                            end
                        elseif (rng >= 0.0 and rng < 0.5) then
                            ...
                            ResourceBuilder.SetResourceType(target_plot_1, 2, 1)
                            __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                                "Nerfed to 1/0/2 Copper Desert Hill");
                            placed_yield = 2
                            if flag == 2 then
                                placed_yield = placed_yield + 1
                            end
                        ...
                        if (target_plot_1:GetResourceCount() < 1 and target_plot_1:GetFeatureType() ~= g_FEATURE_OASIS) then
                            ResourceBuilder.SetResourceType(target_plot_1, 9, 1)
                            __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                                "Nerfed to 2/0 Wheat Desert Floodplains");
                            placed_yield = 2
                            if flag == 2 then
                                placed_yield = placed_yield + 1
                            end
                ...
                elseif (target_plot_1:GetTerrainType() == 9 or target_plot_1:GetTerrainType() == 10 or
                    ...
                    if (target_plot_1:GetFeatureType() ~= g_FEATURE_MARSH) then
                        ...
                        if (rng >= 0.75) then
                            ...
                            ResourceBuilder.SetResourceType(target_plot_1, 7, 1);
                            __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                                "Nerfed to 1/1 Sheep Tundra Hill");
                            placed_yield = 2.5
                        elseif (rng >= 0.5 and rng < 0.75) then
                            -- Copper Hill
                            TerrainBuilder.SetTerrainType(target_plot_1, 10);
                            ...
                            ResourceBuilder.SetResourceType(target_plot_1, 2, 1)
                            __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                                "Nerfed to 1/0/2 Copper Tundra Hill");
                            placed_yield = 2
                        elseif (rng >= 0.0 and rng < 0.5) then
                            -- Forested Deer
                            TerrainBuilder.SetTerrainType(target_plot_1, 10);
                            ...
                            ResourceBuilder.SetResourceType(target_plot_1, 4, 1)
                            __Debug("Terraforming Best X: ", target_plot_1:GetX(), "Y: ", target_plot_1:GetY(),
                                "Nerfed to 1/0/2 Deer Tundra Hill");
                            placed_yield = 2
                        end
                    end
                    ...
                    placed_yield = target_tiles[i].yield
                    __Debug("Terraforming Best Index: ", target_tiles[i].index, "X: ", pPlot:GetY(), "Y: ",
                        pPlot:GetY(), "Couldn't Nerf This Tile");
            ...
            local added_yield = placed_yield - target_tiles[i].yield
            __Debug("Terraforming Best Original Yield: ", target_tiles[i].yield, "Placed Yield ", placed_yield,
                "Remaining Before", remaining_amount, "After", remaining_amount - added_yield);
            remaining_amount = remaining_amount - added_yield
...
function Terraforming_Water(plot, civilizationType)
    ...
    for i = 0, 5 do
        ...
        if (adjacentPlot ~= nil) then
            if (adjacentPlot:GetResourceCount() < 1 and adjacentPlot:IsUnit() == false and adjacentPlot:IsCoastalLand() ==
                false) then
    ...
    for i = 0, 5 do
        ...
        if (adjacentPlot ~= nil) then
            if (adjacentPlot:GetResourceCount() < 1 and adjacentPlot:IsCoastalLand() == false and
                adjacentPlot:GetFeatureType() ~= g_FEATURE_VOLCANO) then
                __Debug("Terraforming Water X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                    "Added: Water Lake but unit was on the way");
                -- TerrainBuilder.SetFeatureType(adjacentPlot,-1);
                -- TerrainBuilder.SetTerrainType(adjacentPlot, 15);
                terraformBBSPlot(adjacentPlot, 15, -2, -2)
                return
    ...
    for i = -1, 17 do
        ...
        if (adjacentPlot ~= nil) then
            if (adjacentPlot:GetFeatureType() == g_FEATURE_FLOODPLAINS or adjacentPlot:GetFeatureType() ==
                ...
                local rng = TerrainBuilder.GetRandomNumber(100, "test") / 100;
                if (rng > limit) then
                    __Debug("Terraforming Floodplains X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                        "Removed: Floodplains");
                    if (i < 6) then
                        TerrainBuilder.SetFeatureType(plot, -1);
                    end
        ...
        if (adjacentPlot ~= nil) then
            if (adjacentPlot:IsWater() == true) then
                ...
                if (tileScore > maxScore) then
                    harborplot_index = i;
                    maxScore = tileScore;
                    __Debug("test harbor X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                        "was picked as harbor plot, score: ", tileScore);
                else
                    __Debug("test harbor X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                        "was not picked as harbor plot, score: ", tileScore);
                end
            end
        end
    end
   ...
   --]]
    ...
    if (harborplot_index ~= nil) then
        ...
        if (harborPlot ~= nil) then
            __Debug("Coastal Terraforming (Step 2) X: ", harborPlot:GetX(), "Y: ", harborPlot:GetY(),
                "Found a valid Harbor tile");
   ...
   --]]
    ...
    elseif (seaCountR1 + seaCountR2 < 3) then -- we have a fjord
        if (seaCountR3 < 4) then -- we have a long fjord
            ...
            if (lakeCountR1 + lakeCountR2 + lakeCountR3 > 3) then -- With the lake, it becomes a standard spawn
                __Debug(
                    "Coastal Terraforming step 3: Found a small fjord with a lake, will consider as standard coastal");
                player.coastalType = COASTAL_START_STANDARD;
                player.isFullCoastal = true;
            else -- long fjord
    ...
    for i = 0, 60 do
        ...
        if (adjacentPlot ~= nil) then
            if (adjacentPlot:GetFeatureType() == 1) then
                __Debug("Costal Terraforming (Step 3a) X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                    "Removing Ice", i);
                TerrainBuilder.SetFeatureType(adjacentPlot, -1);
            end
            ...
            if (adjacentPlot:IsCoastalLand() and isMountain(adjacentPlot)) then
                print("Costal Terraforming (Step 3b) X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                    "found a coastal mountain", i);
                if (isMountainBypassable(adjacentPlot) == false and adjacentPlot:GetFeatureType() ~= g_FEATURE_VOLCANO and
                    adjacentPlot:GetFeatureType() == -1) then
                    print("Costal Terraforming (Step 3b) X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                        "Removing non-bypassable mountain", i);
    ...
    for i = 0, 17 do
        ...
        if (adjacentPlot ~= nil) then
            ...
            if (terrainType == 16) and (adjacentPlot:GetResourceType() == 5 or adjacentPlot:GetResourceCount() < 1) and
                adjacentPlot:IsNaturalWonder() == false then
                __Debug("Terraforming Coastal X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                    "Changing Ocean to Coast tile", i);
                TerrainBuilder.SetTerrainType(adjacentPlot, 15);
            end
        end
    end
    ...
    for i = 18, 35 do
        ...
        if (adjacentPlot ~= nil) then
            ...
            if (terrainType == 16) then
                local rng = TerrainBuilder.GetRandomNumber(100, "test") / 100;
                if (rng > 0.5 and (adjacentPlot:GetResourceType() == 5 or adjacentPlot:GetResourceCount() < 1) and
                    adjacentPlot:IsNaturalWonder() == false) then
                    __Debug("Terraforming Coastal X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                        "Changing Ocean to Coast tile", i);
                    TerrainBuilder.SetTerrainType(adjacentPlot, 15);
    ...
    if (player.coastalType == COASTAL_START_LONG_FJORD or player.coastalType == COASTAL_START_SMALL_FJORD or
        player.coastalType == LAKE_START_BIG) then
    ...
    for i = 0, 17 do
        adjacentPlot = GetAdjacentTiles(plot, i);
        if (adjacentPlot ~= nil) then
            if (adjacentPlot:IsWater() == true and (adjacentPlot:GetFeatureType() == -1) and
                (adjacentPlot:GetResourceCount() < 1) and (adjacentPlot:IsNaturalWonder() == false) and
                isSameTile(harborPlot, adjacentPlot) == false) then
    ...
    if (improvableSeaR2Count == 0) then
        ...
        if (possibleRing2Count == 0) then
            ...
            if (ResourceBuilder.CanHaveResource(upgradedTile, 5)) then
                ...
                __Debug("cleanup Coastal Terraforming (Step 6a) X: ", upgradedTile:GetX(), "Y: ", upgradedTile:GetY(),
                    "Added: Fish");
            end

        end
    else
        __Debug("Cleanup Coastal step 6a: Found ", improvableSeaR2Count,
            " improvable resources on ring 1-2, not adding extra");
    end
    ...
    for i = 18, 35 do
        ...
        if (adjacentPlot ~= nil) then
            if (adjacentPlot:IsWater() == true) and (adjacentPlot:GetFeatureType() == -1) and
                (adjacentPlot:GetResourceCount() < 1) and (adjacentPlot:IsNaturalWonder() == false) then
    ...
    if (improvableSeaResourcesCount >= aimedRessourcesCount) then
        __Debug("Cleanup Coastal step 6b: Found ", improvableSeaResourcesCount,
            " existing resources on ring 1-2-3, not adding extra");

    else
        __Debug("Cleanup Coastal step 6b: Found ", improvableSeaResourcesCount,
            " existing resources on ring 1-2-3, I will add more");
    ...
    while (possibleRing3Count > 0 and improvableSeaResourcesCount < aimedRessourcesCount) do
        ...
        if (ResourceBuilder.CanHaveResource(upgradedTile, 5)) then
            ...
            __Debug("cleanup Coastal Terraforming (Step 6b) X: ", upgradedTile:GetX(), "Y: ", upgradedTile:GetY(),
                "Added: Fish");
        else
            __Debug("cleanup Coastal Terraforming (Step 6b) X: ", upgradedTile:GetX(), "Y: ", upgradedTile:GetY(),
                "Cannot add Fish");
    ...
    if (improvableSeaResourcesCount >= aimedRessourcesCount) then
        ...
        __Debug("Cleanup Coastal step 6b: I only managed to have ", improvableSeaResourcesCount,
            "in total, instead of ", aimedRessourcesCount, "aimed.");
    end
...
function Coastal_plot_Score_R2(element, harborPlot, spawn)
    ...
    if (featureType == g_FEATURE_REEF) then
        ...
        if (resourceType == 51) then
            ...
            -- logs writing
            if (isHarborAdj == true and hasCampusAdj == true) then
                __Debug("Coastal score R1-R2: found turtle with harbor AND campus adj (lucky boy !), counting:",
                    tileScore);
            end
...
function improveCostal(player, aimedNavalScore, margin)
    ...
    while (tile ~= nil and (math.abs(player.coastalScore - aimedNavalScore) > margin)) do
        ...
        if (plot:GetResourceType() == 3 and plot:GetFeatureType() ~= g_FEATURE_REEF) then
            ...
            if (wasChanged == false) then
                ...
                if (player.coastalScore + scoreChange <= aimedNavalScore) then
                    __Debug("Balancing Coastal X: ", plot:GetX(), "Y: ", plot:GetY(),
                        "Changing a crab into a naked fish");
                    setResource(plot, 5, 15);
                    newTileScore = oldTileScore + scoreChange;
                    wasChanged = true;
                else
    ...
    while ((player.possibleCoastalRing2Count > 0 or player.possibleCoastalRing3Count > 0) and
        (aimedNavalScore - player.coastalScore) > margin) do
        ...
        if (player.possibleCoastalRing2Count > 0) then
            ...
            if (aimedNavalScore - (player.coastalScore + newTileScore) >= 0) then
                ...
                player.seaResourcesR2 = {
                    next = player.seaResourcesR2,
                    plot = plot,
                    hasCampusAdj = hasCampusAdj,
                    isHarborAdj = isHarborAdj,
                    score = newTileScore,
                    isLuxury = false
                };
        ...
        if (player.possibleCoastalRing3Count > 0) then
            ...
            if (aimedNavalScore - (player.coastalScore + newTileScore) >= 0) then
                ...
                player.seaResourcesR3 = {
                    next = player.seaResourcesR3,
                    plot = plot,
                    hasCampusAdj = hasCampusAdj,
                    isHarborAdj = isHarborAdj,
                    score = newTileScore,
            ...
            if (r2Element.isLuxury == false) then
                ...
                if (plot:GetResourceType() == 5 and plot:GetFeatureType() == g_FEATURE_REEF) then
                    ...
                    if (wasChanged == false) then
                        ...
                        if (player.coastalScore - aimedNavalScore - scoreChange >= 0) then
                            __Debug("Nerfing Coastal X: ", plot:GetX(), "Y: ", plot:GetY(),
                                "Had Fish+Reef here: Now only reef");
                    ...
                    if (wasChanged == false) then
                        ...
                        if (player.coastalScore - aimedNavalScore - scoreChange >= 0) then
                            __Debug("Nerfing Coastal X: ", plot:GetX(), "Y: ", plot:GetY(),
                                "Had Fish+Reef here: Now only crabs");
                    ...
                    if (wasChanged == false) then
                        ...
                        if (player.coastalScore - aimedNavalScore - scoreChange >= 0) then
                            __Debug("Nerfing Coastal X: ", plot:GetX(), "Y: ", plot:GetY(),
                                "Had Fish+Reef here: Now only Fish");
                ...
                elseif (wasChanged == false and plot:GetResourceType() == 5 and plot:GetFeatureType() ~= g_FEATURE_REEF) then
                    ...
                    if (player.coastalScore - aimedNavalScore - scoreChange >= 0) then
                        __Debug("Nerfing Coastal X: ", plot:GetX(), "Y: ", plot:GetY(),
                            "Had naked Fish here: removing it");
                    ...
                    if (wasChanged == false) then
                        ...
                        if (player.coastalScore - aimedNavalScore - scoreChange >= 0) then
                            __Debug("Nerfing Coastal X: ", plot:GetX(), "Y: ", plot:GetY(),
                                "Had naked Fish here: changing it to Crabs");
...
function mountainToHill(plot)
    if plot:GetFeatureType() ~= g_FEATURE_VOLCANO and plot:IsNaturalWonder() == false then
        ...
        __Debug("mountainToHill X: ", plot:GetX(), "Y: ", plot:GetY(),
            "Cannot Change a Mountain if it is a Volcano or a Natural Wondner");
    end
end
...
function isMountainBypassable(plot)
    ...
    for i = 0, 5 do
        ...
        if (isWalkable(adjacentTile) or (adjacentTile:IsCoastalLand() and isMountain(adjacentTile))) then
            __Debug("Costal Terraforming (Step 3b) X: ", adjacentTile:GetX(), "Y: ", adjacentTile:GetY(),
                "This tile is walkable", i);
            adjacentTiles[i + 1] = true;
        else
            __Debug("Costal Terraforming (Step 3b) X: ", adjacentTile:GetX(), "Y: ", adjacentTile:GetY(),
                "This tile is not walkable", i);
...
function isDistrictable(plot)
    ...
    -- mountains
    -- FIX HERE --
    if (terrainType == 2 or terrainType == 5 or terrainType == 8 or terrainType == 11 or terrainType == 14 or
        terrainType == 15 or terrainType == 16) then
        return false;
    end
...
function Terraforming_Coastal(plot, intensity, post_correction)
    ...
    for i = 0, 17 do
        ...
        if (adjacentPlot ~= nil) then
            ...
            if (adjacentPlot:IsWater() == true and adjacentPlot:GetResourceCount() > 0 and
                adjacentPlot:GetResourceType() ~= 45) then
    ...
    if (post_correction == false) then
        ...
        for i = 0, 17 do
            ...
            if (adjacentPlot ~= nil) then
                if (adjacentPlot:IsWater() == true and adjacentPlot:GetFeatureType() == -1 and
                    (adjacentPlot:GetResourceCount() < 1 or adjacentPlot:GetResourceType() == 5)) and
                    adjacentPlot:IsNaturalWonder() == false then
                    if (count_resources < 3) and adjacentPlot:GetResourceCount() < 1 then
                        if (ResourceBuilder.CanHaveResource(adjacentPlot, 5)) then
                            ...
                            ResourceBuilder.SetResourceType(adjacentPlot, 5, 1);
                            TerrainBuilder.SetTerrainType(adjacentPlot, 15);
                            __Debug("Coastal Terraforming (Step 3) X: ", adjacentPlot:GetX(), "Y: ",
                                adjacentPlot:GetY(), "Added: Fish");
                        end
                    end
                    if (rng > limit_1 and count_reefs < 2) and adjacentPlot:IsFreshWater() == false then
                        __Debug("Coastal Terraforming X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                            "Added: Reef");
                        TerrainBuilder.SetFeatureType(adjacentPlot, g_FEATURE_REEF);
                        TerrainBuilder.SetTerrainType(adjacentPlot, 15);
                        count_reefs = count_reefs + 1;
                        local rng = TerrainBuilder.GetRandomNumber(100, "test") / 100;
                    elseif (((rng / count_resources > limit_2) or (count_resources < 3)) and
                        adjacentPlot:GetResourceType() == -1) then
                        if (ResourceBuilder.CanHaveResource(adjacentPlot, 5)) then
                            count_resources = count_resources + 1
                            ResourceBuilder.SetResourceType(adjacentPlot, 5, 1);
        ...
        for i = 0, 60 do
            ...
            if (adjacentPlot ~= nil) then
                terrainType = adjacentPlot:GetTerrainType();
                rng = TerrainBuilder.GetRandomNumber(100, "test") / 100;
                if (terrainType == 16) and rng > limit and
                    (adjacentPlot:GetResourceType() == 5 or adjacentPlot:GetResourceCount() < 1) and
                    adjacentPlot:IsNaturalWonder() == false then
                    __Debug("Terraforming Coastal X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                        "Changing Ocean to Coast tile", i);
                    TerrainBuilder.SetTerrainType(adjacentPlot, 15);
                    local rng = TerrainBuilder.GetRandomNumber(100, "test") / 100;
                    if (adjacentPlot:GetFeatureType() == -1 and rng > limit and adjacentPlot:GetResourceType() == -1 and
                        ((count_resources < 3 and i < 17) or (count_resources < 4 and i > 30)) and
                        (post_correction == false) and adjacentPlot:IsFreshWater() == false) then
                        TerrainBuilder.SetFeatureType(adjacentPlot, g_FEATURE_REEF);
                        __Debug("Coastal Terraforming (Step 4) X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                            "Added: Reef", i);
                        count_resources = count_resources + 1;
                        local rng = TerrainBuilder.GetRandomNumber(100, "test") / 100;
                        if ((rng / count_resources / count_resources) > limit and adjacentPlot:GetResourceType() == -1) then
                            -- Reef with fish 
                            __Debug("Coastal Terraforming (Step 4) X: ", adjacentPlot:GetX(), "Y: ",
                                adjacentPlot:GetY(), "Added: Fish");
                            ResourceBuilder.SetResourceType(adjacentPlot, 5, 1);
                        end
                    end
                    ...
                    if (adjacentPlot:GetFeatureType() == 1 and rng > limit / 2) then
                        __Debug("Costal Terraforming (Step 4) X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                            "Removing Ice", i);
                        TerrainBuilder.SetFeatureType(adjacentPlot, -1);
                    end
                end
        ...
        if (adjacentPlot ~= nil) then
            if adjacentPlot:IsNaturalWonder() == false then
                ...
                if ((terrainType == 9) and rng > limit and flag ~= 1) then
                    __Debug("Terraforming X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                        "Changing Tundra to Plains tile", i);
                    -- TerrainBuilder.SetTerrainType(adjacentPlot,g_TERRAIN_TYPE_PLAINS);
                    terraformBBS(adjX, adjY, 3, -2, -2);
                    rng = TerrainBuilder.GetRandomNumber(100, "test") / 100;
                    if world_age == 1 and adjacentPlot:GetResourceCount() == 0 and adjacentPlot:GetFeatureType() < 4 and
                        rng < 0.20 then
                        __Debug("Terraforming X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                            "Make it a Plains hill", i);
                ...
                if ((terrainType == 10) and rng > limit and flag ~= 1) then
                    __Debug("Terraforming X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                        "Changing Tundra Hills to Plains Hills tile", i);
                    -- TerrainBuilder.SetTerrainType(adjacentPlot,g_TERRAIN_TYPE_PLAINS_HILLS);
                    terraformBBS(adjX, adjY, 4, -2, -2);
                    if adjacentPlot:GetFeatureType() == -1 and adjacentPlot:GetResourceCount() == 0 and rng < 0.15 then
                        __Debug("Terraforming X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(), "Add woods", i);
                ...
                if ((terrainType == 11) and rng > limit and flag ~= 1) then
                    __Debug("Terraforming X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                        "Changing Tundra Mountains to Plains Mountains tile", i);
                    -- TerrainBuilder.SetTerrainType(adjacentPlot,5);
                    terraformBBS(adjX, adjY, 5, -2, -2);
                end
                if ((terrainType == 6) and rng > limit and flag ~= 2) then
                    __Debug("Terraforming X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                        "Changing Desert to Plains tile", i);
                    -- TerrainBuilder.SetTerrainType(adjacentPlot,g_TERRAIN_TYPE_PLAINS);
                    terraformBBS(adjX, adjY, 3, -2, -2);
                ...
                if ((terrainType == 7) and rng > limit and flag ~= 2) then
                    __Debug("Terraforming X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                        "Changing Desert Hills to Plains Hills tile", i);
                    -- TerrainBuilder.SetTerrainType(adjacentPlot,g_TERRAIN_TYPE_PLAINS_HILLS);
                    terraformBBS(adjX, adjY, 4, -2, -2);
                end
                if ((terrainType == 8) and rng > limit and flag ~= 2) then
                    __Debug("Terraforming X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                        "Changing Desert Hills to Plains Hills tile", i);
                    -- TerrainBuilder.SetTerrainType(adjacentPlot,5);
                    terraformBBS(adjX, adjY, 4, -2, -2); -- Talasta ERROR should change to mountains
                end
                if (terrainType == 12) then
                    if (i < 18 and flag ~= 1) then
                        __Debug("Terraforming X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                            "Changing to Plain tile", i);
                        -- TerrainBuilder.SetTerrainType(adjacentPlot,g_TERRAIN_TYPE_PLAINS);
                        terraformBBS(adjX, adjY, 3, -2, -2);
                    elseif (i < 36 and flag == 1) then
                        __Debug("Terraforming X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                            "Changing to Tundra tile", i);
                ...
                if ((terrainType == 4) and rng > limit and adjacentPlot:GetFeatureType() ~= g_FEATURE_JUNGLE and flag ~= 1 and flag ~= 2) then
                    __Debug("Terraforming X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                        "Changing to Grassland Hills tile", i);
                    -- TerrainBuilder.SetTerrainType(adjacentPlot,1);
                ...
                if ((terrainType == 1) and adjacentPlot:GetResourceCount() < 1 and flag == 2 and rng < 0.66) then
                    __Debug("Terraforming X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                        "Changing Grassland Hills to Plains Hills tile", i);
                    -- TerrainBuilder.SetTerrainType(adjacentPlot,4);
                    terraformBBS(adjX, adjY, 4, -2, -2);
                end
                if ((terrainType == 2) and adjacentPlot:GetResourceCount() < 1 and flag == 2) then
                    __Debug("Terraforming X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                        "Changing Grassland Mountains to Plains Mountains tile", i);
                    -- TerrainBuilder.SetTerrainType(adjacentPlot,5);
                    terraformBBS(adjX, adjY, 5, -2, -2);
                end
                if ((terrainType == 3 or terrainType == 4 or terrainType == 5) and flag == 2) then
                    ...
                    for k = 0, 5 do
                        ...
                        if adjacentPlot2 ~= nil then
                            if adjacentPlot2:GetTerrainType() == 6 or adjacentPlot2:GetTerrainType() == 7 or
                                adjacentPlot2:GetTerrainType() == 8 then
                                d_count = d_count + 1
                            end
                        end
                    end
                    if d_count > d_factor then
                        __Debug("Terraforming X: ", adjacentPlot:GetX(), "Y: ", adjacentPlot:GetY(),
                            "Changing Plains to Desert tile", i);
                        -- ResourceBuilder.SetResourceType(adjacentPlot, -1);
                ...
                rng = TerrainBuilder.GetRandomNumber(100, "test") / 100;
                if (adjacentPlot:IsWater() == false and adjacentPlot:IsImpassable() == false and
                    adjacentPlot:GetTerrainType() ~= 12 and adjacentPlot:GetTerrainType() ~= 13 and
                    adjacentPlot:GetTerrainType() ~= 6 and adjacentPlot:GetTerrainType() ~= 7 and
                    adjacentPlot:GetFeatureType() == -1 and rng > limit_tree and adjacentPlot:GetResourceType() == -1 and
                    count_wood < max_wood) then
                    terraformBBS(adjX, adjY, -2, -2, 3);
                    -- TerrainBuilder.SetFeatureType(adjacentPlot,3);
...
function RemoveProd(plot)
    ...
    for j = 0, 17 do
        ...
        -- __Debug("Evaluate Start X: ", otherPlot:GetX(), "Evaluate Start Y: ", otherPlot:GetY(), "Terrain Type: ", terrainType);
        if otherPlot ~= nil then
            if (otherPlot:GetTerrainType() == 4 or otherPlot:GetTerrainType() == 1) and otherPlot:GetResourceType() ==
                -1 then
...
function RemoveFood(plot)
    ...
    if rng > 0.5 then
        for j = 0, 17 do
            ...
            -- __Debug("Evaluate Start X: ", otherPlot:GetX(), "Evaluate Start Y: ", otherPlot:GetY(), "Terrain Type: ", terrainType);
            if otherPlot ~= nil then
                if (otherPlot:GetResourceType() == 0 or otherPlot:GetResourceType() == 1 or otherPlot:GetResourceType() ==
                    6 or otherPlot:GetResourceType() == 9) then
                    __Debug("Food balancing: Food Removed", otherPlot:GetResourceType());
                    ResourceBuilder.SetResourceType(otherPlot, -1);
                    return true;
                end
                if ((otherPlot:GetFeatureType() == 2 or otherPlot:GetFeatureType() == 5) and
                    otherPlot:GetResourceCount() < 1) then
                    TerrainBuilder.SetFeatureType(otherPlot, -1);
...
function Terraforming_BanLux(plot)
    for j = 0, 6 do
        ...
        if otherPlot ~= nil then
            __Debug("Check Luxury Start X: ", otherPlot:GetX(), "Evaluate Start Y: ", otherPlot:GetY(),
                "Resource Type: ", otherPlot:GetResourceType());
            if (otherPlot:GetResourceType() == 31 or otherPlot:GetResourceType() == 33 or otherPlot:GetResourceType() ==
                16 or otherPlot:GetResourceType() == 12 or otherPlot:GetResourceType() == 25) then
    ...
    for j = 0, 30 do
        ...
        if otherPlot ~= nil then
            __Debug("Check Luxury Start X: ", otherPlot:GetX(), "Evaluate Start Y: ", otherPlot:GetY(),
                "Resource Type: ", otherPlot:GetResourceType());
            if (otherPlot:GetResourceType() == 11 or otherPlot:GetResourceType() == 27 or otherPlot:GetResourceType() ==
                28) then
                __Debug("Luxury balancing: Banned Luxury Removed", otherPlot:GetResourceType());
                -- ResourceBuilder.SetResourceType(otherPlot, -1);
                terraformBBS(adjX, adjY, -2, -1, -2)
    ...
    if MapConfiguration.GetValue("BBSStratRes") == 3 then
        ...
        if (isIron == false) then
            for _, element in ipairs(ring13) do
                ...
                if (terrainTile < 15 and featureTile ~= 0 and featureTile ~= 31 and featureTile ~= 32 and terrainTile %
                    3 ~= 2 and resourceTile == -1) then
        ...
        if (isHorse == false) then
            for _, element in ipairs(ring13) do
                ...
                if (terrainTile < 5 and featureTile ~= 0 and featureTile ~= 31 and featureTile ~= 32 and terrainTile % 3 ~=
                    2 and resourceTile == -1) then
        ...
        if (isAluminium == false) then
            for _, element in ipairs(ring15) do
                ...
                if ((terrainTile > 2 and terrainTile < 9) and featureTile ~= 0 and featureTile ~= 31 and featureTile ~=
                    32 and terrainTile % 3 ~= 2 and resourceTile == -1) then
        ...
        if (isOil == false) then
            for _, element in ipairs(ring15) do
                ...
                if (terrainTile < 15 and terrainTile % 3 ~= 2 and resourceTile == -1) then
                    ...
                    -- cleaning feature
                    if (featureTile ~= -1 and featureTile ~= 0 and featureTile ~= 31 and featureTile ~= 32 and
                        featureTile ~= 5) then
                        TerrainBuilder.SetFeatureType(localPlot, -1);
                        mapFeatureCode[xIndex][yIndex] = -1;
                    end
    ...
    for row in GameInfo.StartBiasFeatures() do
        if (row.CivilizationType == civilizationType) then
            if row.FeatureType ~= nil then
                if row.FeatureType == "FEATURE_FLOODPLAINS" or row.FeatureType == "FEATURE_FLOODPLAINS_GRASSLAND" or
                    row.FeatureType == "FEATURE_FLOODPLAINS_PLAINS" then
                    return true
                end
            end
        end
                    ...
                    if (adjacentPlot4 ~= nil) then
                        if (adjacentPlot4:GetX() >= 0 and adjacentPlot4:GetY() < gridHeight) then
                            ...
                            if (adjacentPlot5 ~= nil) then
                                if (adjacentPlot5:GetX() >= 0 and adjacentPlot5:GetY() < gridHeight) then
                                    if (i + 1 == 6) then
                                        adjacentPlot2 = Map.GetAdjacentPlot(adjacentPlot5:GetX(), adjacentPlot5:GetY(),
                                            0);
                                    else
                                        adjacentPlot2 = Map.GetAdjacentPlot(adjacentPlot5:GetX(), adjacentPlot5:GetY(),
                                            i + 1);
