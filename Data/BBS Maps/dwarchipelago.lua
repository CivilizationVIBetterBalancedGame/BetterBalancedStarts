------------------------------------------------------------------------------
--	FILE:	 DWArchipelago.lua
--	AUTHOR:  EvilVictor (Seven05)
--	PURPOSE: Map script - Produces widely varied islands.
------------------------------------------------------------------------------
...
function GeneratePlotTypes(world_age)
	...
	while done == false do
		...
		-- 705: Always create the initial continents using the high sea level, when the water
		-- layer is added below that will correct the amount of water.
		iWaterThreshold = g_continentsFrac:GetHeight(80);
		...
		-- 705: Shift the plots here before adding islands
		...
		if (land_percent_plus >= target_percent and land_percent_minus <= target_percent + 10) then
			-- Now test the biggest landmass
			if (iNumBiggestAreaTiles <= iNumTotalLandTiles * 0.15 and iNumBiggestAreaTiles >= iNumTotalLandTiles * 0.10) then
				done = true;
	...
	print("- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -");
	print("--- Detailed Archipelago landmass generation, Attempt#", iAttempts);
