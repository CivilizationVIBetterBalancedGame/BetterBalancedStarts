------------------------------------------------------------------------------
--	FILE:	 DWMixedLand.lua
--	AUTHOR:  EvilVictor (Seven05)
--	PURPOSE: Map script - Produces widely varied continents.
------------------------------------------------------------------------------
...
function GenerateMap()
	print("Generating Mixed Land+ Map");
	local pPlot;
...
function GeneratePlotTypes(world_age)
	...
	while done == false do
		...
		iWaterThreshold = g_continentsFrac:GetHeight(water_percent - 15);
		...
		-- 705: Inserting water layer from the multilayered fractal.
		-- For Mixed Land maps we'll do this in regions with different grain in each so
		-- the first thing is to mix up the grain values, nothing fancy
		
		grain1 = 2 + TerrainBuilder.GetRandomNumber(5 - grain_dice, "Continental Grain roll - LUA Mixed Land");
		if (grain1 == 3) then
			grain2 = 4;
		else
			grain2 = 3;
		...
		detail_water = math.max(math.min(20 + detail_water - (5 * grain1),90),70);
		
		if (grain1 == 4) then
			-- 705: 1st and 2nd Quadrant combined
			local args = {};
			args.iWaterPercent = detail_water;
			args.iRegionWidth = math.ceil(g_iW * 0.5);
			args.iRegionHeight = math.ceil(g_iH);
			args.iRegionWestX = math.floor(0);
			args.iRegionSouthY = math.floor(0);
			args.iRegionGrain = grain1;
			args.iRegionPlotFlags = g_iFlags;
			...
			args.iWaterPercent = detail_water;
			args.iRegionWidth = math.ceil(g_iW * 0.5);
			args.iRegionHeight = math.ceil(g_iH * 0.25);
			args.iRegionWestX = math.floor(g_iW * 0.5);
			args.iRegionSouthY = math.floor(g_iH * 0.5);
			args.iRegionGrain = 2;
			args.iRegionPlotFlags = g_iFlags;
			args.iRegionFracXExp = 7;
			...
			args.iWaterPercent = detail_water;
			args.iRegionWidth = math.ceil(g_iW * 0.5);
			args.iRegionHeight = math.ceil(g_iH * 0.5);
			args.iRegionWestX = math.floor(0);
			args.iRegionSouthY = math.floor(0);
			args.iRegionGrain = grain1;
			args.iRegionPlotFlags = g_iFlags;
			...
			args.iRegionHeight = math.ceil(g_iH * 0.5);
			args.iRegionWestX = math.floor(0);
			args.iRegionSouthY = math.ceil(g_iH * 0.5);
			args.iRegionGrain = grain2;
			args.iRegionPlotFlags = g_iFlags;
			args.iRegionFracXExp = 7;
			args.iRegionFracYExp = 6;
			args.iRiftGrain = 2;
		...
		if (removedLand < iNumTotalTiles / 100) then
			...
			if (land_percent_plus >= target_percent and land_percent_minus <= target_percent + 10) then
				...
				-- or an island map
				if (iNumBiggestAreaTiles <= iNumTotalLandTiles * 0.6) then
					done = true;
	...
	print("- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -");
	print("--- Detailed Mixed Continents landmass generation, Attempt#", iAttempts);
	...
	for x = 0, g_iW - 1 do
		for y = 0, g_iH - 1 do
			...
			if(plotDataIsCoastal[i] == false) then
				...
				if(hillCount < rChance - 3 and plotTypes[i] == g_PLOT_TYPE_LAND) then
					plotTypes[i] = g_PLOT_TYPE_HILLS;
					hillsAdded = hillsAdded + 1;

				-- Add mountain or remove hill in hilly areas
				elseif(plotTypes[i] == g_PLOT_TYPE_HILLS) then
					if(hillCount > rChance + 1 and mountainsAdded < mountainsAllowed) then
						plotTypes[i] = g_PLOT_TYPE_MOUNTAIN;
...
function AddFeatures()
	...
	args.iJunglePercent = 44; --36
	args.iForestPercent = 22; --24
